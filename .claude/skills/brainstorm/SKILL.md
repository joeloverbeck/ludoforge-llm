...
1. **Step 1.2 classification announcement**: Preserved. The classification announcement is a transparency gate (forces conscious classification, creates an audit trail), not an interview gate. Announce as the first sentence of the compound-move message even when zero interview rounds occur.
...
**Section-preview gate**: If Step 2 used the compound-move variant and the user's response did not contain an explicit waiver phrase from the Step 4 enumeration ("just write the file", "skip the preview", "no need to walk me through it", "go ahead and write"), send the section-bullet preview now as its own message before writing the artifact. "As its own message" here means separate from the Write tool call — bundling the section bullets in the same prose message as a brief approach recap is permitted; what is not permitted is sending the section bullets only inside (or immediately alongside) the Write call. This applies even under "no clarifying questions" (which constrains interview rounds, not transparency gates per Pre-Set Directives §4). This gate also fires on the Step 3 foreclosed path (user's invocation directly specifies the artifacts) because the Step 2 compound-move still applies there per Step 3's foreclosure paragraph; the absence of an approach presentation does NOT exempt the artifact write from the section-bullet preview. Skip this gate when plan mode is active — the plan file is itself the preview surface. After the Write call lands, proceed to Step 6 — the next-steps menu (or auto-mode recommendation under "no clarifying questions") is still required; Step 5 is not the terminal step.
