...
5. **Write output artifacts** per the mode-specific routing. Load `references/output-artifacts.md`. Covers all five output dispositions (design / decision-triage / operational / in-place documentation edit / multi-file-directory), the section-preview gate, the numbering and four-path replacement conventions, the Reassessment-section requirement for source-derived outputs (external LLM proposals AND internal decision-handoff reports), the interdependent multi-spec ordering-index rule and the independent-specs user-requested ordering-index rule, phased-spec acceptance budgets, destructive-action sections, and the Design Dismiss outcome path.
...
7. **Present next steps.** Load `references/next-steps-menu.md`. Covers the per-output-type menus (design doc / spec / triage tickets / triage spec+report / operational plan / measurement report / multi-file directory / new skill), namespace derivation, multi-spec and multi-phase adaptations, mixed primary + secondary mutation overlay, Continual Learning prompt evaluation, and auto-mode adaptation. **Continual Learning MUST be evaluated when ANY of:**
   - (a) brainstorm output contradicts a `CLAUDE.md` / `docs/FOUNDATIONS.md` / skill / `reports/` claim.
   - (b) brainstorm supersedes a prior report's load-bearing recommendation.
   - (c) brainstorm produces the named follow-up artifact a reference report explicitly deferred to — the offer in this case is to append a `Follow-up: <spec/ticket>` breadcrumb to the source report. **This is the most common trigger** (per the frequency note in `next-steps-menu.md`: measurement-report → follow-up-spec is the common brainstorm flow).

   Skipping this evaluation is a common miss when reference loading is shortcut.
...
- **Reference loading is mandatory**: Each numbered Procedure step requires loading its named `references/<file>` before executing the step's behavior — even in high-confidence flows where the topic feels familiar. The Procedure-step summaries are pointers to the load-bearing rules, not substitutes for them. Skipping reference loading is the most common cause of missed transparency gates and missed Continual Learning offers in this skill. **Compound-move front-loading**: when the compound-move variant is chosen (the default for Decision-requiring-design / external-LLM flows), Steps 3–5 (and often 7) collapse into ~2 assistant turns with no intervening reference-loading beat — batch-load `interview-protocol.md`, `approaches-and-design.md`, `output-artifacts.md`, and `next-steps-menu.md` at the point the compound-move is chosen, rather than lazily per step where the collapsed cadence will skip them.
- **Section-bullet preview is non-waivable under compound-move**: When Step 2 used the compound-move variant (High-Confidence Start, auto-mode, or "no clarifying questions"), the section-bullet preview MUST land before the first Write tool call (or, when the deliverable is in-place edits to existing files rather than a new artifact, before the first Edit call) — one preview covering all planned files in a multi-file output satisfies the gate. "No clarifying questions" constrains interview rounds, not transparency gates. Bundling the section bullets in the same prose message as the approach + findings recap is permitted; what is forbidden is sending the bullets in the same assistant message as the Write call — the preview MUST be in an earlier message (the Write lands in a later message) so the user can object before the file is written. Prose bullets that merely precede the Write call within one assistant message do NOT satisfy the gate. **Concretely**: end the assistant turn after the section-bullet preview — no Write or Edit tool call in the same response; the Write lands in the next assistant turn triggered by user input or an auto-mode tick. Treat the gate as a turn-yield constraint, not just an ordering constraint. See `references/output-artifacts.md` Section-preview gate (including the "Procedural counterpart — yield turn after preview" sub-rule) and `references/interview-protocol.md` Pre-Set Directives §4 for the canonical specification.
