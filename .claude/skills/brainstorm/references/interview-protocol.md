...
**Compound-move variant at 80–94%**: When the remaining gaps are all multiple-choice terminal decisions (which approach, is scope X in or out, amend foundation Y), the Step 3 approach presentation and the final gap-closer questions may be combined into a single message — the user's choice of approach simultaneously resolves the remaining gaps. This is the natural flow when the gaps are "which option" rather than "what's the problem", and the approach recommendations already implicitly argue for one scope/amendment answer over the others. The message shape: short findings recap → 2-4 approaches with tradeoffs → explicit batched gap-closers ending with "pick one and call out the other gaps". When the batched gap-closers are presented via `AskUserQuestion`, the natural shape is a single call with multiple questions (the tool supports 1-4 questions per call) rather than sequential calls — bundle independent terminal decisions (approach + scope tweak, disposition + namespace) into one call when their answers don't depend on each other. See Interview Rule 1's terminal-round exception. If the user rejects the terminal `AskUserQuestion` to clarify the framing or the option set rather than answer it, treat the rejection as a clarification round — the design-mode analog of Mid-Flow Investigation (Triage Mode) below — not a confidence regression: answer the clarification, then re-present the same approaches (revised only if the clarification changed them). For high-stakes or philosophically-loaded architecture decisions, a brief discursive exchange before the terminal prompt is often warranted.
...
4. **Step 4 Compound-move + auto-mode intersection**: The directive does NOT waive the section preview. "Without stopping for clarifying questions" constrains interview rounds, not transparency gates. The section-bullet preview is still required as a separate message before the artifact write (this matches the existing disqualification clause in Step 4). "Separate message" here — and everywhere in this skill — means the section preview MUST be in an assistant message that does not contain the Write tool call (the Write lands in a later message); bundling section-name bullets in the same prose message as approach + findings recap is permitted and often efficient under this directive — what the preview must NOT share a message with is the Write call itself, though it need not be its own standalone prose message. Prose bullets that merely precede the Write call within one assistant message do NOT satisfy the gate.
5. **Step 6 next-steps menu**: Present as an inline recommendation, no `AskUserQuestion`. The "no clarifying questions" directive treats the Step 6 menu the same as auto-mode adaptation — inline recommendation + redirect opportunity. The Continual Learning offer, when it fires, is an additional *option in the inline recommendation list* — not a separate prose paragraph preceding the list, and not a separate `AskUserQuestion` question (the "separate question to avoid false mutual exclusion" rule in Step 6 applies only when AskUserQuestion is the presentation mechanism). Concretely: if the inline recommendation lists numbered or bulleted next-step options (e.g., "1. Decompose into tickets / 2. Review first / 3. Done"), the CL offer is one of those numbered/bulleted items (e.g., "4. Append a 'Follow-up: <spec>' breadcrumb to <source-report>"), not a "Want me to also …?" paragraph appended after the list. See Step 6 auto-mode adaptation for the inline recommendation shape.
