...
1. **Resolve the PR**.
   ...
   - If omitted: `gh pr view --json number,headRefName,headRepository,statusCheckRollup,isCrossRepository` (auto-detects from current branch). If this fails, abort and ask the user for a PR number. If multiple open PRs exist and auto-detect failed (e.g., the current branch is `main`), prefer `AskUserQuestion` with the candidate PRs (number + branch + updatedAt) as options over a free-text prompt — the answer is enumerable.
...
4. **Enumerate failing & timing-out checks**.
   - If `statusCheckRollup` from Step 1.1 gave you all (check-name, run-id, job-name) tuples needed downstream, skip to the per-job-classification sub-bullet — the rollup is sufficient.
   - Otherwise (e.g., you need run-level fields like `headSha` or `createdAt` for the brand-new-lane check in Step 3): `gh pr checks <N>` for the rollup status, and `gh run list --branch <headRefName> --limit 20 --json databaseId,name,status,conclusion,headSha,workflowName,jobs` to map check names to run IDs.
...
5. **Download failed logs**.
   - For each failed run/job: `gh run view <run-id> --log-failed > /tmp/ci-fix-<run-id>.log`. **For cancelled runs, `--log-failed` returns empty because no step failed before the runner was killed — and occasionally a FAILED (not cancelled) run also returns empty/non-zero, when the failing step's logs are not captured in the `--log-failed` slice. In either case, capture the full log via `gh run view <run-id> --log > /tmp/ci-fix-<run-id>-full.log` instead.** Cancelled runs are most often timeouts — see Step 3's timeout sub-classification.
   - **Parallel-download orchestration**: when downloading multiple logs in parallel (common for clusters with many failing lanes), create the target directory in a SEPARATE sequential step BEFORE launching the parallel batch — e.g., run `mkdir -p /tmp/ci-fix-pr<N>/` as its own command first, then issue the parallel `gh run view ... > /tmp/ci-fix-pr<N>/<file>.log & ... & wait` batch as a separate command. A compound shell that chains `mkdir -p ... && <parallel-batch>` can lose redirects: backgrounded jobs may race ahead of filesystem visibility for the just-created directory, and `> /tmp/<dir>/<file>` silently fails with `No such file or directory` for some (not all) of the parallel writes. Use absolute paths throughout the parallel batch — the Bash tool's cwd does NOT propagate to backgrounded jobs, so a relative path that depends on a `cd` in the same compound shell will resolve against the wrong directory.
...
| `engine-perf.yml` | `perf` | `pnpm -F @ludoforge/engine test:perf` (CI builds `engine-wasm` + `engine` first; targets `dist/test/perf/**`) |
...
**`perf` vs `performance` — distinct lanes.** `engine-perf.yml` (job `perf`, script `test:perf`, dir `dist/test/perf/**`) and `engine-tests.yml`'s `test (performance)` (script `test:performance`, dir `dist/test/performance/**`) are different lanes over different directories. Match the failing check name exactly — running `test:performance` for a `perf` failure (or vice versa) exercises a disjoint test set and can produce a false "did not reproduce" verdict.

**Unknown lanes**: read the relevant `.github/workflows/*.yml`, locate the job by name, and extract the run command from the matrix entry. Treat the YAML as the source of truth — never rely on stale skill-internal mappings. Convention shortcut: GitHub Actions check names for `engine-tests.yml` matrix lanes embed the matrix params in parentheses as `test (<id>, <script>, <timeout>)` — the second param is the npm script, which often gives the local repro (`pnpm -F @ludoforge/engine <script>`) without opening the YAML. Validate against the YAML when in doubt. Fastest of all for single-script lanes: the failed log's npm pre-script echo prints the exact command CI ran — lines of the form `> @scope/pkg@ver <script>` followed by `> <resolved command>` (e.g., `> @ludoforge/engine@0.0.0 test:perf` then `> node --test "dist/test/perf/**/*.test.js"`) — an authoritative repro source that needs no YAML lookup.
...
- `OOM / allocation-pressure` — V8 heap exhausted during a long-running lane (`FATAL ERROR: Ineffective mark-compacts near heap limit Allocation failed - JavaScript heap out of memory`). Distinct from `timeout` (the process was actively allocating, not stuck) and from the `test:memory` lane (which has its own dedicated budget). Often cross-cuts many lanes (perf, performance, determinism shards, policy-canaries, etc.) because a single hot-path allocation regression surfaces wherever that path is exercised. See the `OOM` playbook entry below.
...
- **≥1 result**: pick the most-recent `success` SHA as the last-green-main reference. Use a fresh `git worktree add /tmp/<worktree-name> <last-green-main-sha>`, build the engine, and run the failing test's local repro there. Last-green-main is also the bisect lower bound; merge-base (where the PR diverged) is only relevant when the PR has diverged significantly and you need to understand the inheritance window. **Shortcut**: when that most-recent `success` run belongs to the same workflow that contains the failing lane (and the lane is not `continue-on-error`), CI's verdict that the lane passed on main is already authoritative — the local worktree reproduction is confirmatory only and may be skipped, especially when last-green-main == merge-base. Reserve the worktree rebuild for when you need to inspect the failure locally or doubt the CI signal.
...
- **Pickaxe instead of bisect for schema/identifier drift**: when the regression's surface is a specific identifier (a now-required field, a removed export, a renamed function), `git log --all -S '<identifier>' -- <implicated-file>` (pickaxe search) locates the introducing commit in one shot — preferable to bisect when the symptom is greppable. Bisect remains the right tool for trajectory shifts whose symptom is a numeric assertion or behavioral difference, not a string.
...
Read `docs/FOUNDATIONS.md` if not already read in this session. Validate every proposed fix against it. **Per-citation verification**: for each Foundation cited in the diagnosis (e.g., "Foundation 8"), verify the section number against the current `docs/FOUNDATIONS.md` — section numbering can shift when principles are added or removed, and prior-conversation recall is not sufficient (see Guardrails: FOUNDATIONS hard halt).
...
Status values: `PR regression` (lane was green on main, broken by this PR) | `pre-existing on main` (the failure is caused by main commits, not by the PR — includes both (a) lane currently observed-red on main and (b) lane destined to fail on the next main run because a main commit between last-green-main and current main HEAD introduced the regression but the lane has not yet triggered; reproduction on current main HEAD confirms the classification in both sub-cases) | `structural` (lane is brand new or budget never validated; needs shard/timeout adjustment) | `flake-suspect` (verified by 3x local re-run).
...
**Deferral off-ramp.** Iteration is not unbounded digging. When investigation reveals a cluster whose fix is *not a localizable patch* — a determinism-sacred (Foundation 8) or multi-layer design defect, or one that needs design intent you do not have — present **defer-to-ticket/spec** as an explicit gate-1 option rather than continuing to re-investigate. Deferral means: the cluster stays diagnosed-but-unfixed, its lanes remain red on the next CI run, the other (landable) clusters are fixed and pushed, and the deferred fix is captured as a ticket (see Step 9's "Deferred-cluster ticket handoff") or a spec (Step 10). Surface this off-ramp the moment a cluster's fix shape stops shrinking across re-gate rounds — do not keep prototyping a determinism-sacred fix the user has not scoped.
...
- **Workflow literal ↔ meta-test sync**: When editing `.github/workflows/*.yml`, grep `packages/engine/test/unit/lint/` (or equivalent meta-test directory) for the literal value being changed (lane name, timeout value, script path). Meta-tests that pin those literals for taxonomy enforcement will fail on the next CI cycle if the meta-test isn't updated in the same edit. Step 7's sentinel grep targets diagnostic instrumentation, not literal-pinning drift, so this co-evolution check must happen at edit time, not at staging time.
...
4. If it still fails, two sub-cases:
   - **(a) Refine within the same fix shape** — the gate-1-approved approach is right but needs a parameter tweak, additional file touched, or narrower scope. Revise inline, rebuild, re-verify; no re-gate needed because the shape the user approved is unchanged.
   - **(b) Pivot to a different fix shape** — the gate-1-approved approach is insufficient and a structurally different fix is needed (e.g., user approved "memoize" but the work isn't bounded enough; need to escalate to "cap per-call budget" or "redesign the data path"). Return to Step 4 (gate 1) with the revised cluster proposal — the user did not see this shape at gate 1, so explicit re-approval is required even under auto-mode. Document the prior shape's verification result in the re-gate message so the user can decide whether the new shape supersedes or complements.
   - Do NOT proceed to commit until every non-advisory failing lane passes locally — except (i) per the environment-constrained verification rule below, or (ii) for a cluster the user has explicitly chosen to **defer** to a ticket or follow-up PR (per Step 4's deferral off-ramp). A deferred cluster's lanes may remain red at push time, provided the deferral is recorded (ticket reference + the chronic-PR report) and the gate-2 message states explicitly which lanes will stay red on the new CI run and why. This differs from the environment-constrained exception, which still expects the deferred lane to pass on CI; a deferred cluster is *not expected to pass* until its ticket lands.

**Re-gate vs. gate-2 disclosure (two carve-outs from the Step-4 re-gate requirement).** Step 6.4(b) and the masking branch below default to re-gating at Step 4 when the fix diverges from what the user approved. Two narrow cases instead permit proceeding with **explicit gate-2 disclosure** of the divergence (so the user can still object before push):
- **Approved mechanism infeasible, same goal reachable another way** — the gate-1-approved fix *mechanism* proves impossible to implement as specified, but an alternative achieves the *same approved goal/invariant* (e.g., approved "materialize X in WASM" is infeasible, yet degrading X to an existing exact TS fallback keeps the approved `wasmRouteCount>0` invariant green). The goal is unchanged; only the internal mechanism differs.
- **Identical-pattern cascade** — an unmasked cluster shares the *same root cause and `@test-class`* as a cluster the user approved (and authorized the *approach* for) earlier this session; it MAY be folded under that approval rather than re-gated.

In both cases the gate-2 message MUST state what diverged from the gate-1 approval (the mechanism changed / the cluster was folded). If the goal itself changed, or the new shape is not an identical-pattern sibling, re-gate at Step 4 — no carve-out.
...
**Lane granularity — a single-file pass is not a lane pass.** When a lane runs multiple test files sequentially and halts at the first failing file (e.g., `run-tests.mjs` returns on the first non-zero exit), the original CI failure log shows only that first file — later files in the lane never ran, so their pass/fail state is unknown. Fixing the first failing file can therefore *unmask* a later-file failure that surfaces only in CI on the next push (this is the halt-at-first-error masking case above, but at file-across-lane granularity rather than within one test run). Verifying just your changed file (or just your diff) does NOT establish the lane is green: run the full lane's local repro — the Step 2 command, which covers every file in the lane — before claiming it verified. If you scope below lane granularity under the environment-constrained rule below, state explicitly at gate 2 that later files in the lane are deferred to CI and may surface a masked failure.

**Environment-constrained verification**: If running a lane locally has previously crashed/hung the development environment in this session (e.g., WSL2 hang, OOM kill) OR the lane's CI duration is >15min AND its heaviness is structural (large determinism / property-test sweeps, multi-minute tournament runs), the user may approve a scoped verification: run faster sibling tests + final `pnpm turbo lint typecheck` + a fresh `pnpm -F @ludoforge/engine build`, and rely on CI for the heavy lane itself. Surface this trade-off explicitly in the gate 2 message — list which lanes were verified locally and which were deferred to CI, with the reason. This is a deliberate scoped exception, not a general bypass: every lane that CAN be verified locally without environment risk MUST still be verified locally. **Serialize heavy local runs — do not parallelize them**: in a resource-constrained environment (e.g., WSL2 with limited memory), run heavy verification lanes one at a time rather than backgrounding several concurrently; concurrent heavy suites can OOM or hang the environment even when each would succeed alone.
...
**`ci`-job step-level masking — run the WHOLE job sequence, not just the failing step.** The `ci` job in `ci.yml` is a SINGLE GitHub job that runs `pnpm turbo lint` → `typecheck` → `build` → `test` sequentially under `shell: bash -e` and halts at the first failing step. Step 2's table lists these as four "lanes," but they are one job: a `lint`, `typecheck`, or `build` failure means CI never reached the later steps, so those steps' failures are masked exactly as a halt-at-first-error pipeline masks later test files (the masking branch above, but at job-step granularity rather than file-in-lane granularity). When the failing `ci` step was `lint`, `typecheck`, or `build`, re-running only that step does NOT verify the job — run the full sequence locally: `pnpm turbo lint typecheck build test`. Treat any failure this unmasks as a new cluster per the "verification SUCCEEDS but reveals a NEW failure" branch above.
...
1. Stage the edits explicitly by file path (avoid `git add -A` / `git add .` per security rule). After staging, run a sentinel grep across the staged diff for common instrumentation patterns: `git diff --cached -- packages/ | grep -nE 'process\.stderr\.write|console\.log|DIAG_|@diag-' | head -20`. Run this unconditionally — the cost is one tool call and catches drift even on tiny diffs (e.g., a one-line edit that happens to contain a leftover `console.log`). Any hits other than pre-existing call sites should be removed before continuing to gate 2 — this backstops the Step 4 hypothesis-prototype cleanup rule programmatically. If the prototype involved ad-hoc `.mjs` scripts, also run `git status --short | grep -E '\.mjs$'` and confirm they are not in the staged set.
...
**Chronic-PR-record trigger check (flag it now, not at Step 9).** Count the distinct clusters resolved this round — gate-1 clusters plus any unmasked during Step 6. If the count is ≥ 4 (Step 9's trigger (a)), state in the gate-2 message that a `reports/ci-failures-pr-<N>-*.md` record is due in Step 9, so the post-push summary does not drop it once attention shifts to the happy path.
...
After push, the skill closes with three concerns: present the final summary to the user, decide whether to write a chronic-PR record, and recognize that CI may surface masked failures requiring re-entry.
...
- New CI run URL — fetch via `gh run list --branch <head> --json headSha,workflowName,status,url` filtered to the just-pushed HEAD SHA (`git rev-parse HEAD`); `gh pr checks <N>`'s rollup lags the new push and can return the prior run, and `gh run list --limit 1` is ambiguous across workflows. Poll briefly if the new run has not registered yet, then present the URL itself in the summary (not the command).
...
Do not commit additional artifacts. The skill is conversational — diagnostic output stays in the transcript.

#### When to write a chronic-PR record

Write `reports/ci-failures-pr-<N>-YYYY-MM-DD.md` summarizing the diagnosis table for future reference when ANY of these conditions hold:

- **(a)** Cluster count is ≥ 4 in this round. (Calibration: when every cluster was fully resolved and locally verified with no lanes deferred to CI, the record largely duplicates the transcript + commit body — still write it, but it carries the most value when lanes were deferred to CI or clusters remain unresolved across pushes.)
- **(b)** ≥ 2 prior pushes on this branch have targeted the same still-failing lane (chronic-PR case — prior fix attempts did not resolve the lane). Detect via `git log --oneline <merge-base>..HEAD` for ≥ 2 recovery commits whose subjects start with `fix:` / `test:` / `Implemented ` AND reference that workflow / lane — match the subject shapes Step 7 actually produces, not a `fix(` conventional-scope prefix. "Same lane" refers to the user-visible CI check-name (e.g., `policy-canaries`), not specific test files within the lane: different masked failures in the same shard across rounds count as the same lane for chronic-PR purposes.
- **(c)** A prior `reports/ci-failures-pr-<N>-*.md` file already exists for this PR — append a new dated report rather than starting fresh, and explicitly note in its "what did NOT work" section whether the prior cluster's root cause is still active or has been since fixed by intervening commits.

For the chronic-PR case, the record should preserve the gate 1 cluster table verbatim plus a one-paragraph "what did NOT work" summary citing the prior fix commits, so the next session can pick up where this one left off without re-bisecting from scratch.

#### Deferred-cluster ticket handoff

When the user defers a diagnosed cluster's fix (per Step 4's deferral off-ramp) to a ticket rather than fixing it in this PR, author the ticket(s) per repo convention so the handoff is self-contained — mirroring Step 10's spec-authoring discipline, but for the `tickets/` workflow:

- Start from `tickets/_TEMPLATE.md`; satisfy the mandatory sections it and `tickets/README.md` require (`Assumption Reassessment (YYYY-MM-DD)`, `Architecture Check`, `Acceptance Criteria`, `Test Plan`).
- Pick a descriptive ticket prefix not already used under `tickets/` or `archive/tickets/`.
- The `**Deps**` line must contain only bare existing paths — no parentheticals or trailing annotations. `pnpm run check:ticket-deps` parses each comma-separated entry as a path and fails on a `path (note)` form; put any Foundation/rule citation in the ticket body, not the Deps line. Run `pnpm run check:ticket-deps` to validate before finishing.
- Embed the deferred cluster's captured evidence (the diagnosis, instrumentation findings, and a "what did NOT work" list) so the ticket needs no re-bisecting. The chronic-PR report (above) and the ticket should cross-reference.
- Leave the ticket untracked for the user's own review/commit boundary — do not stage or commit it in the CI-fix push unless the user explicitly asks (same rule as Step 10 specs).
- The final summary must name the deferred cluster, the lanes that stay red, and the ticket file(s) created.

#### Multi-round CI recovery

If the next CI run on the just-pushed SHA surfaces new failures, treat them as a fresh skill entry starting from Step 1 against the updated run IDs. Each round's environment-constrained verification deferrals (Step 6) are the most common source of CI-time masking, because lanes deferred to CI never had their full failure surface observed locally. Round-N diagnosis tables should explicitly note `masked by round-(N-1) failures` in the cluster status column so a reader of the gate-1 message can trace the cascade. The chronic-PR record-writing rule above applies once round count compounds the trigger conditions.

CI-time masking is the common case in this skill's recurrence pattern — distinct from the local-verification masking in Step 6's "verification SUCCEEDS but reveals a NEW failure" branch, which fires before push. Both kinds of masking can compound in a single recovery: a local-verification unmask may itself surface a CI-time unmask on the next push.

### Step 10: Architectural-Gap Scan (post-push)

Hot-fixes for CI failures often paper over an architectural gap rather than close it. After the push lands, scan the just-pushed commit's diff (`git show HEAD`, or `git diff <base>...HEAD` for the whole branch) for any of these patterns — by this point the changes are committed and pushed, so nothing is staged and `git diff --staged` would be empty. Each is a candidate architectural gap that warrants a follow-up spec in `specs/<NNN>-<slug>.md`:
...
- **Pre-compiled fixture or cached artifact not validated against its source by CI** (e.g., a checked-in compiled GameDef JSON, a vendored schema snapshot, a regenerable lookup table). Often signals that the regen script has a `check` mode that's never wired into CI — the artifact silently drifts from the source spec / kernel schema until a downstream consumer breaks. The architectural fix is a check-mode CI lane (1-minute typical), not perpetual manual regen. Reference shape: `.github/workflows/bootstrap-fixtures.yml`.

For each pattern observed, name the gap (one short sentence), suggest a candidate spec number (the next free `specs/` index — verify the candidate isn't already used in `archive/specs/` either, since spec numbers are unique across both directories in this repo) and slug, and offer to write the spec. Use existing specs as the format template (e.g., `specs/148-runtime-interned-identifier-comparison.md` or `specs/149-fitl-evolution-readiness-numeric-substrate-bytecode-vm.md`). Do NOT auto-write specs without user approval — produce hints; leave authorship to the user. If the user accepts the offer, draft the spec file directly via Write and leave it untracked for the user to review and commit; do not stage or commit the spec in the same push as the CI fix unless the user explicitly asks, since the spec is a discrete review surface that benefits from its own commit boundary. If the user redirects the architectural-gap follow-up to an immediate direct fix (e.g., "just add the CI lane" or "fix it in code now") rather than a spec, treat it as a user-directed action outside the skill's recovery contract — follow normal git discipline (explicit-path staging, fresh commit, verification, push). The skill's PR recovery is complete; the redirection is a separate, user-owned change.

This step is post-push and additive: the fix is already shipped. The architectural-gap follow-up is a separate effort, not a blocker for closing CI failures. The pattern list above is illustrative, not exhaustive — surface a novel gap shape too (e.g., a downstream consumer re-deriving data the kernel already exposes authoritatively, such as re-implementing seat→profile binding resolution instead of reading `def.agents.bindingsBySeat`), generalizing the listed patterns rather than skipping because no listed pattern matched exactly. If genuinely no gap of any shape is present, say so and skip — false positives here would dilute the signal.
...
- **Golden-trace re-bless mechanism**: When a `@test-class: golden-trace` test's fixture has drifted from a deliberate (and user-approved at gate 1) behavior change, the standard re-bless pattern is an `UPDATE_GOLDEN=1` env mode added to the test. Pattern:
  1. Read `const UPDATE_GOLDEN = process.env.UPDATE_GOLDEN === '1';` at module scope.
  2. Inside the `it`, capture the actual output the test would assert against.
  3. If `UPDATE_GOLDEN`, write the captured output to the fixture file via `writeFileSync` and `return` (skip the assertion).
  4. Otherwise read the expected fixture and `assert.deepEqual` / `assert.equal` against the captured output.

  Then build the engine and run with `UPDATE_GOLDEN=1 node --test dist/test/<path>/<test>.test.js`, verify the fixture diff is the expected trajectory shift, then run again without the env var to confirm the test passes against the new fixture.

  **Dynamic-index variant**: When the fixture pins a *position* in the trace (e.g., `decisionIndex: 314`) that may shift under a trajectory change, the UPDATE_GOLDEN branch should also rediscover the index — scan the captured trace for the first decision matching the test's shape predicates (kind, preview utility, ref ids, etc.) and write the new index alongside the new snapshot. The shape predicates should be extracted into a single helper called by both the assertion and the scan, so re-bless and normal mode validate against the same predicate set.

  **Commit-body marker**: The re-bless commit body MUST include `Re-bless golden trace: <test-file>` per `.claude/rules/testing.md` Update Protocol, plus a human-readable reason naming the spec or implementation change that caused the trajectory shift. Step 7's commit-subject prefix (`test: retarget <…>`) signals to a reviewer that the diff is a trajectory re-alignment; the body marker is what makes the re-bless explicit to future readers and `git log`-based audits.
...
- **OOM / allocation-pressure**: V8 heap-out-of-memory mid-lane (not the dedicated `test:memory` budget). Symptom is `FATAL ERROR: Ineffective mark-compacts near heap limit Allocation failed - JavaScript heap out of memory` in the log, often after a long warm-up period during which the lane was making progress. Reproduce locally at a smaller heap (`node --max-old-space-size=512 --test <file>`) to force the OOM faster (~30s vs ~3min at CI's 4GB). For diagnosis, use V8's statistical profiler (`node --prof <command>`; survives SIGKILL/SIGABRT) since `--cpu-prof` does not flush before V8 aborts; `node --prof-process isolate-*.log` summarizes bottom-up. The most common cause in this codebase is a new feature added to a proposer/agent/kernel hot path that was previously cheap, now allocating per (template × root × decision-step) without per-call memoization or per-caller budget caps. Fix shapes (in order of invasiveness): (1) per-call memoization keyed on a fingerprint of the inputs; (2) tight per-caller budget cap that bounds the work without losing correctness (e.g., `maxDecisionProbeSteps: 4` instead of the default 128 when the call site is advisory); (3) lighter algorithm that avoids the heavy machinery entirely (e.g., single-shot probe instead of full continuation analyzer). When clustering, treat OOMs across many lanes as one cluster if the same hot path is implicated by recent commits — they typically share a root cause. To validate the hypothesis cheaply, temporarily comment out the suspected hot-path call in a single file (per Step 4's hypothesis-validation-prototype allowance), rebuild, and confirm the lane completes; revert before applying the real fix.
...
- **Two gates always**: gate 1 (pre-fix) and gate 2 (pre-push) are mandatory. Auto mode does not waive them — auto mode is "not a license to destroy". **User-authorized shortcut on re-entry**: when the user invokes the skill on a still-failing PR with explicit directive language equivalent to "find and apply the solutions" (typical when CI surfaces masked failures and the user asks the assistant to handle them), the gate-1 menu may be presented inline (cluster table + per-cluster proposed fix shape + foundations cited) without an `AskUserQuestion` prompt, and gate-2 may be folded into the standard end-of-turn summary. The structural requirement remains: gate 1 = present diagnosis before edit; gate 2 = present diff and commit subjects before push. The presentation mode (AskUserQuestion vs inline reply) is the variation, not the gating discipline. When the user's directive language is itself an answer to a gate-1 sub-menu (e.g., "find solutions" is the "chase" option for a flake-suspect's three-option menu of chase / retry CI / skip), select the option the directive most plainly chooses and cite that interpretation inline. If multiple options remain defensible under the directive, surface the menu rather than picking silently. **Scope of the shortcut — what it does NOT cover**: the shortcut covers selecting among gate-1-presented options and re-entry on a still-failing PR. It does NOT cover a fix-shape pivot that introduces a new fix path the user did not see at gate 1 (per Step 6.4(b)) — that requires explicit re-gate at Step 4, even under auto-mode, because the user's prior approval was scoped to the shape they saw. (Two narrow carve-outs permit gate-2 disclosure in lieu of a Step-4 re-gate — the approved-mechanism-infeasible and identical-pattern-cascade cases in Step 6's "Re-gate vs. gate-2 disclosure" note — but only when the divergence is explicitly flagged at gate 2.)
- **Files NOT touched**:
