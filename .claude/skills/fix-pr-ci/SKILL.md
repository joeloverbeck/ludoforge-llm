...
**Or — if verification SUCCEEDS for the original cluster but reveals a NEW failure that was masked by the prior one** (common in build pipelines that halt at the first error: lint masking a build break, one Node-only import masking another, one failing test masking the next): treat the new failure as a new cluster, append it to the diagnosis, and re-gate at Step 4 with the additional cluster. Each round of unmasking advances the recovery; do not commit until verification reveals no further masked failures. The "Lighter-weight variant" stash technique in Step 3 is the right tool for distinguishing whether a newly-surfaced failure was caused by your edits or pre-existed in PR HEAD.
