...
**Base-branch resolution.** All `main` references in this step (and in Step 9's chronic-PR detection) resolve to `origin/main`. Run `git fetch origin main` before any `git merge-base`, `git worktree add`, or `git bisect` against main — local `main` may lag the remote by several commits, which would silently change the merge-base, the bisect lower bound, and the "PR-introduced vs main-introduced" classification.
