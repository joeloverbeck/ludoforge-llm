...
**Verification probe**: when a directive or a diagnostic question can only be answered by instrumenting the mutable surface (adding a temporary probe consideration, feature, or flag to generate diagnostic artifacts), do so as a *probe*, not an experiment: apply the instrumentation, run the harness or a trace tournament, aggregate the result, then `cd $WT && git checkout -- <mutable-files>` to revert. A verification probe is NOT logged to `results.tsv` and does NOT increment `experiment_count`; record its purpose and result in the DIAGNOSE musings entry. (If a probe surfaces an engine/runtime/DSL gap, that routes to Step 7.7 — see its trigger framing.)
...
- **Autonomous loop** (default — no substantive user dialogue in the past few turns): write the report first, then pause. Follow numbered steps 1-5 below.
- **Dialogue-active** (Human Investigation Interrupt active, or substantive conversational input within the past few turns): summarize the gap in conversation FIRST, write the report AFTER the user confirms scope. See the trailing **Dialogue-active variant** paragraph below for specifics. Musings + results.tsv entries (steps 3-4) still land before resuming, regardless of mode.

Then in either mode:
1. If the gap was surfaced by an experiment: classify that experiment per the normal Step 7 LOG status (typically `REJECT`, occasionally `CRASH`); the architectural-gap halt does NOT change the experiment's classification — it is an additional pause-and-report decision after the experiment is logged. If the gap was surfaced by a DIAGNOSE / verification probe (not an experiment): there is no experiment row to classify — the `arch-gap-NNN` marker row in step 4 is the only row.
