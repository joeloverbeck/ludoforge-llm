...
3. Read `$WT/campaigns/<campaign>/results.tsv` — if it has data rows beyond the header, resume from the last accepted state (the current HEAD of the worktree branch IS the last accepted state).
   - **Stale-baseline recovery**: if the recorded baseline commit hash from `checkpoints.jsonl` is not reachable from the worktree HEAD (verify with `git merge-base --is-ancestor <baseline-hash> HEAD`; non-zero exit = unreachable), the prior worktree was removed and the recorded state no longer matches the worktree branch. **Determine the restart sub-case** before resetting state by comparing the worktree branch's mutable files (from program.md's Mutable System section) against the `INITIAL_SEED_TIER` bootstrap state (typically the unmodified clone-of-baseline defined in program.md's Bootstrap section). Two sub-cases: **STATE-FRESH** (mutable files match the bootstrap, or you cannot distinguish — default safe path) — treat as a true fresh restart: clear `results.tsv` to the header row, clear `checkpoints.jsonl`, reset `seed-tier.txt` to `INITIAL_SEED_TIER`, and proceed to Phase 1 (Baseline) as if no prior data existed. **STATE-EVOLVED** (mutable files differ materially from the bootstrap — e.g., the evolved profile carries tier-N considerations, parameters, or preview config that the bootstrap did not have) — the mutable surface reflects prior session work that the recorded baseline commit no longer pins; clear `results.tsv` and `checkpoints.jsonl` as in STATE-FRESH, but **keep `seed-tier.txt` at its current value** and re-baseline at that tier in Phase 1. Resetting to `INITIAL_SEED_TIER` in the STATE-EVOLVED case would waste iterations re-ratcheting through already-cleared tiers against an already-evolved mutable surface. Document the reason and sub-case in musings: `**STALE BASELINE**: Prior worktree removed; recorded baseline <hash> not reachable from new HEAD. Restarting as <STATE-FRESH | STATE-EVOLVED>; seed-tier <reset to N | retained at N>.`
   - **Clone-style bootstrap caveat**: for campaigns whose Bootstrap step *clones an evolved profile from a baseline control* (e.g., FITL ARVN's `arvn-evolved` cloned from `arvn-baseline`), the STATE-FRESH/STATE-EVOLVED determination is based on the *evolved* profile's content, not the baseline control's. Read the evolved profile in the mutable file: (a) if it does not exist, the Bootstrap step never ran — treat as STATE-FRESH and run Bootstrap now; (b) if it exists and its content materially differs from the baseline control (extra considerations, tuned params, deeper preview config), treat as STATE-EVOLVED — prior session work is preserved, regardless of any inference about whether a prior "Campaign Completion" was executed; (c) if it exists with content matching the baseline control (a bootstrap clone that hasn't been mutated yet), treat as STATE-FRESH. **Never** infer prior-session-completion status from the *baseline control's* content alone — baselines are immutable by design across the campaign lifecycle (per program.md's controls list), so a control profile carrying anything beyond its original simple form is a programming error, not a signal that Campaign Completion ran. The Suspended-campaign case is the typical source of the STATE-EVOLVED-with-evolved-profile state; never confuse it with the (rarer) post-completion re-bootstrap state.
   ...
   - **Crash / interruption recovery** (worktree intact, an in-progress operation was interrupted — e.g., an environment crash or a killed process): unlike Stale-baseline recovery (which handles a *removed* worktree), here the worktree and branch survive, but background processes are dead and the working tree may carry residue from an interrupted IMPLEMENT or verification probe. Recover before resuming: (1) `cd $WT` and verify cwd; (2) `git status --short` — working-tree residue from an interrupted IMPLEMENT/probe should be `git checkout`-reverted unless it was a committed ACCEPT; (3) confirm no orphaned harness/worker processes linger; (4) re-derive strategy state per the Strategy state recovery bullet above, then resume at Step 0 ANCHOR.
...
timeout "$HARD_TIMEOUT" bash campaigns/<campaign>/harness.sh   # wall-clock hang guard; if HARD_TIMEOUT is `unlimited`, omit the `timeout` wrapper
```

Apply this preflight at every harness call site: Phase 1 baseline (step 1 above), Step 4 EXECUTE in `references/harness-execution.md`, ceiling-baseline runs after a tier promotion or phase transition, post-meta-review trial runs. The `timeout "$HARD_TIMEOUT"` wrapper (default 1800s — see `references/prerequisites-and-config.md`) is part of the canonical preflight: a run exceeding `HARD_TIMEOUT` is killed and logged `EARLY_ABORT` per Step 4 EXECUTE's "Wall-clock hang abort". The paren-subshell form `(cd "$WT" && bash campaigns/<campaign>/harness.sh)` is a stricter inline equivalent that scopes the `cd` to a child shell and never leaks cwd back to the parent — useful for one-off harness probes from inside a longer diagnostic sequence.

**Background launch**: harness runs are typically multi-minute. Launch them with `run_in_background: true` to avoid blocking the conversation; the runtime delivers a background-task-completion notification when the command finishes. Do NOT schedule a wakeup or sleep-poll while waiting — polling burns context budget for no benefit. Awaiting a background-task-completion notification is the *sanctioned* wait — it does NOT violate the "never pause" autonomy directive. After launching a background harness or diagnostic, yield the turn; the runtime re-invokes you when it completes. The wait is not a "natural stopping point" and not a permission request — it is the loop's normal cadence for multi-minute work.
...
After every 10 experiments OR after any strategy ladder advance OR after any Step 7.7 architectural-gap-halt that you *considered substantively but rejected* (e.g., a trace finding that initially looked architectural but Step 1b's verification gate dismissed as a known config trade-off, or a partial-coverage borderline trigger you elected not to fire) — NOT the default per-iteration "no trigger fired" branch — OR in response to substantive user concern or question that requests visibility (distinct from a USER NUDGE which elevates a new hypothesis — that goes through Step 2 instead), the agent MAY emit a brief status summary to the conversation (1-3 paragraphs: experiments completed, accept/reject counts, current strategy, top finding). The agent does NOT halt — the next iteration begins immediately after the summary. The status is informational, not a permission request. The human may interrupt to redirect; absent redirection, the loop continues. This complements the autonomy directive ("never seek permission") with a communication channel that does not violate it.
...
**Diagnostic protocol**:
1. Read trace/diagnostic artifacts produced by the harness (traces, logs, profiles). Also enumerate any prior-session helper scripts in the campaign folder (e.g., `diagnose-*.mjs`, `regen-*.mjs`) — they often implement reusable trace aggregation that this session's DIAGNOSE step can call directly instead of re-implementing. The "After Campaign Completes" runtime-file preservation explicitly retains these scripts across worktree removal for exactly this kind of reuse.
...
**Verification gate (before acting on a trace finding)**: when a diagnostic reading would imply an architectural problem (preview broken, picks counter-game-theoretical, signal direction inverted, agent ignoring a config flag) or a strategy change, verify the interpretation by re-reading the canonical source fields BEFORE acting. Trace summaries (utility, range, count, breakdown stats) are derived; the canonical fields are the source of truth — for example, `selectedX` rather than `candidates[0].X`, the full per-candidate score breakdown rather than aggregate stats. If practical, run a controlled comparison experiment that should produce a known-different result; outcome divergence confirms the interpretation. The cost of one verification round is small; the cost of a misclassified architectural-gap halt or a retracted finding is high. **The gate is symmetric**: verify before *dismissing* a candidate architectural gap as a non-issue (a config trade-off, inherent complexity, expected scaling) just as rigorously as before *raising* one. A premature dismissal recorded in musings or a lesson is as expensive as a premature halt — it must later be retracted via Lesson Correction when disproven, and may cost a user round-trip in the meantime.

**Verification probe**: when a directive or a diagnostic question can only be answered by additional measurement — either **(a) instrumenting the mutable surface** (adding a temporary probe consideration, feature, or flag to generate diagnostic artifacts) OR **(b) re-running the harness or tournament with different arguments** (e.g., enabling per-seed trace capture via `--trace-default all`, increasing verbosity, or invoking an existing diagnostic helper script in the campaign folder) — do so as a *probe*, not an experiment: apply the instrumentation (or run the alternate-arg invocation), aggregate the result, then `cd $WT && git checkout -- <mutable-files>` to revert any mutable-surface changes (the alternate-arg variant has nothing to revert). A verification probe is NOT logged to `results.tsv` and does NOT increment `experiment_count`; record its purpose and result in the DIAGNOSE musings entry. (If a probe surfaces an engine/runtime/DSL gap, that routes to Step 7.7 — see its trigger framing.)
...
- **Anti-pattern check (pre-flight invariance / triggering / scope sanity):** Before proposing a hypothesis to IMPLEMENT, sanity-check that the change can actually alter the agent's argmax on some decision. Common dead-weight patterns to avoid:
  - **Scalar invariance**: multiplying a single consideration's weight by a constant preserves argmax when that consideration is the sole driver of the signal it carries. To shift behavior via weight tuning, change RELATIVE weights between competing considerations (e.g., projectedMarginWeight vs governWeight ratio), not absolute scale on one.
  - **Rare-trigger conditionals**: estimate from prior traces or state-feature values what fraction of decisions a conditional `when` clause would fire on. If <5% across the seed corpus, the consideration is likely dead weight — pick a different threshold or a different state-feature.
  - **Silent-at-scope**: a new candidateFeature reading `preview.feature.<name>` is silent if the underlying state-feature does not change within preview depth for the targeted action class (e.g., control flips after subsequent Coup-phase resolution, not within depthCap=16 of a main-phase Train placement). Verify the signal actually fires at the targeted scope before committing the experiment.
  - **Forced-action targets**: a consideration that contributes only on actions the agent has no real choice over (single-candidate frontiers, coup-phase forced redeploys) cannot alter outcomes. Verify `appliesToActions` or the consideration's scope reaches decisions with multiple competing candidates.

  If any anti-pattern matches the proposed change, propose a different change. This 30-second pre-flight saves ~10 min of harness time per false-positive experiment.
...
- `EARLY_ABORT`: harness was killed mid-run — either for exceeding `ABORT_THRESHOLD` (a metric overshoot; the row carries the running metric value) or for exceeding `HARD_TIMEOUT` (a wall-clock hang; no metric was produced, so the `metric_value` column is the literal `null`, mirroring the un-retryable-CRASH convention above).
...
Trigger this step before Step 8 (REPEAT) when the just-completed experiment — **or a Step 1b DIAGNOSE / verification probe** — surfaces an engine/runtime/DSL gap rather than a campaign-mutable-surface limitation. (The trigger conditions below are written experiment-centrically, but apply equally to a gap surfaced by a DIAGNOSE-phase probe; a probe-surfaced gap has no experiment row — the `arch-gap-NNN` marker from step 4 is the only row.) This is the agent-initiated counterpart to the Human Investigation Interrupt's engine-limitation discovery (step 5 of that protocol) and to Step 1g's Ceiling Detection — but it can fire after a single experiment, without waiting for `CEILING_THRESHOLD` non-accepts, when the evidence is definitive. This step operationalizes Foundation 15 (Architectural Completeness): silent no-ops, partial-coverage gaps, and documented-but-non-functional capabilities are root-cause issues that must be reported as specs or reports rather than worked around.
...
**On trigger**, first **detect mode**:
- **Autonomous loop** (default): no user message within the last 3 turns AND no Human Investigation Interrupt active.
- **Dialogue-active**: **(a)** ANY user message within the last 3 turns, regardless of content (correction, clarification, redirect, status question); **OR (b)** a Human Investigation Interrupt is active and has not been resolved by a user-directive resume.

Then execute the numbered steps below. Step 3 (write the report) has a **mode-dependent reordering**:
- **Autonomous mode**: execute steps 1-6 in numerical order.
- **Dialogue-active mode**: execute steps 1, 2, 4, 5 first (classify, lesson cross-check, musings entry, results.tsv marker), then *pause for the user to confirm scope by summarizing the gap in conversation*, then execute step 3 (write the report) once scope is confirmed, then step 6 (formal AskUserQuestion). The reordering prevents an over-broad or off-scope report from being written and then re-scoped.

Musings + results.tsv entries (steps 4 and 5) land before the user-pause regardless of mode.

1. If the gap was surfaced by an experiment: classify that experiment per the normal Step 7 LOG status (typically `REJECT`, occasionally `CRASH`); the architectural-gap halt does NOT change the experiment's classification — it is an additional pause-and-report decision after the experiment is logged. If the gap was surfaced by a DIAGNOSE / verification probe (not an experiment): there is no experiment row to classify — the `arch-gap-NNN` marker row in step 5 is the only row.
2. **Lesson cross-check (mandatory before writing the report)**: grep `lessons.jsonl` (this campaign's) and `campaigns/lessons-global.jsonl` (cross-campaign) for prior lessons describing the same architectural pattern. If a prior `type: architectural` or `type: negative` lesson exists, the report (step 3 below) MUST cite it and either (a) re-verify with this session's new evidence and frame the finding as "prior lesson still holds under <new engine state>" (e.g., "STILL HOLDS post-spec-178"), or (b) extend the prior finding with the new evidence rather than starting from scratch. Pure re-confirmation is still report-worthy when prior-session lessons were captured before a significant engine change, but the report must position itself relative to the prior lesson, not as fresh discovery. Document the cross-check result inline in the musings entry (step 4 below) — either "no prior lesson; fresh discovery" or "prior lesson <source_exp / quoted text>; this finding re-verifies / extends it".
3. Write a report (`reports/<topic>-<date>.md`) or spec (`specs/<NNN>-<name>.md`) in the main repo root (not the worktree). Use the same spec-vs-report criterion from Human Investigation Interrupt step 5 (single clear fix → spec; multi-gap research → report). The artifact MUST include:
   ...
   - **Cross-reference to any prior lesson** identified by step 2's cross-check, with the framing chosen there ("still holds" / "extends").

   If the report or spec creation precedes a degenerate-flow squash-merge (the typical autonomous-loop case), see After Campaign Completes step 7's pre-merge timing branch — pre-merge artifacts are referenced by file path or commit hash in the squash-merge commit message. If there is no squash-merge to carry the reference (typical "Suspended after arch-gap (no branch diff)" sub-case — see After Campaign Completes), commit the spec/report directly to main as a standalone `docs:` commit with resume-contract language. **Spec authoring convention**: when a report is written first and a follow-up spec next, cite the report as a "Trigger reports:" entry in the spec's frontmatter header (see spec 169 §1 / spec 170 §1 for the format). This preserves the audit trail from empirical evidence to design proposal to implementation.
4. Append to musings: `**ARCHITECTURAL-GAP HALT**: <one-line summary>. See <report or spec path>. Halting at exp-NNN.`
5. Append to results.tsv: `arch-gap-NNN	<best_metric>	0	architectural-gap	REJECT	architectural gap discovered at exp-NNN; see <artifact path>` (this is a marker row similar to ceiling-NNN, not a new experiment).
6. **Pause for human input**: Use AskUserQuestion with the four-option taxonomy from Step 1g Ceiling Detection, scoped to the single-experiment halt context:
   - **Option A** — Human provides a `next-idea.md` → resume the loop with that hypothesis.
   - **Option B** — Halt the campaign → proceed to "After Campaign Completes" (typically the "Suspended after arch-gap (no branch diff)" degenerate-flow sub-case).
   ...
   - **Option D** — Architectural spec or report creation per step 3 above; pair with halt-and-resume-after-spec if the unblock requires implementation work (see After Campaign Completes "Suspended Campaign Resume" for the resume contract).
...
2. Copy gitignored runtime files back to the source campaign folder (see step 4 of the **Normal-campaign flow below** for the canonical preservation list and the tracked-musings caveat; the Normal flow's runtime-file preservation steps apply here too — return to this sub-section's step 3 after preserving). The musings.md from a degenerate campaign contains diagnostic history (investigation notes, baseline analysis, root cause findings) that may be valuable when the campaign is restarted after the blocking issue is resolved — preserve it. **If `musings.md` is tracked in this campaign**, do not blindly overwrite the main repo's committed version; either skip the copy or merge intentionally (the main repo's tracked musings may have been deliberately edited or truncated by the user).
3. **Pre-merge check**: Verify the branch has commits worth keeping: `git diff main...improve/<campaign> --stat`. If no meaningful diff remains (all changes were reverted in the working tree, or only invalid/diagnostic commits exist), skip the squash-merge — but **do not skip the spec/report commit** if step 4's middle or third branch applies.
4. **Determine the commit shape based on the branch's state**, choosing exactly ONE of three branches:
   - **(a) Branch has useful infrastructure commits OR `campaigns/lessons-global.jsonl` was modified this campaign** (e.g., a `type: negative` correction obsoleting a prior global lesson, or any other infrastructure-shaped lesson update): switch to the main repo root and squash-merge. **If `lessons-global.jsonl` was modified, commit it on the worktree branch first per the normal-flow step 3** (`git add -f campaigns/lessons-global.jsonl && git commit -m "chore: promote global lessons from <campaign>"`) so the squash-merge picks it up. Commit the squash-merge with a summary noting the campaign was halted due to `<reason>` and listing infrastructure changes. If specs/reports were created during this session, reference them in the squash-merge commit message (per the spec/report timing branch below). Skip the metric impact summary.
   - **(b) Branch has no meaningful diff AND specs/reports were created this session** (the typical Step 7.7 arch-gap halt sub-case — see "Suspended after arch-gap (no branch diff)" below): skip the squash-merge entirely, and instead commit the new specs/reports directly to main as a standalone `docs:` commit. The commit message MUST satisfy the Suspended Campaign Resume contract (name the unblocking spec, e.g., `Campaign <name> suspended pending Spec NNN implementation`) since there is no squash-merge to carry that resume-contract language. This is the only branch where the spec/report needs to be committed via a standalone main-tree commit rather than as part of a squash-merge.
   - **(c) Branch has no meaningful diff AND no specs/reports were created**: skip the commit step entirely. Nothing to land; proceed to worktree removal.

   The "skip lesson promotion" guidance applies to forward-looking ACCEPT-driven lesson promotion only; retroactive corrections to `lessons-global.jsonl` persist regardless of campaign outcome (branch (a) above handles them).
5. Remove the worktree and delete the branch (step 9 of the Normal-campaign flow below).

**Suspended after arch-gap (no branch diff)** — a Step 7.7 → Option B/D halt where specs/reports were written this session but no infrastructure or accepted-experiment commits accumulated on the worktree branch. This is a *named sub-case* of the Degenerate-campaign flow above, with the typical mapping:
- Degenerate step 1: spec/report were already created during Step 7.7's step 3 (no further work).
- Degenerate step 2: preserve runtime files per the Normal-flow step 4 reference.
- Degenerate step 3: the pre-merge check confirms no meaningful diff.
- Degenerate step 4 **branch (b)**: standalone `docs:` commit with resume-contract language naming the unblocking spec.
- Degenerate step 5: remove worktree.
- The **Suspended Campaign Resume** contract (below) applies when the spec eventually lands.

This sub-section is a named entry point so a reader who arrives via Step 7.7's Option B/D handoff can confirm the expected shape end-to-end without re-deriving it from the Degenerate flow's branching.

**Suspended Campaign Resume** (a campaign halted with an architectural unblock pending — e.g., Step 7.7 Option D where the user halts the campaign and commissions a follow-up spec that must land before the loop can produce signal). Suspension is orthogonal to degenerate-vs-normal: a campaign of *either* shape can be suspended. A **degenerate** suspended campaign uses the Degenerate-campaign flow above; a **normal** suspended campaign (one or more accepted experiments) uses the Normal-campaign flow below — in both cases, add the resume-contract steps in this section:

1. The squash-merge (Degenerate- or Normal-campaign, per the campaign's shape) captures the infra fixes, accepted-experiment commits if any, lessons-global update, and spec/report references; this is the correct terminal shape, not a separate flow. The campaign is "suspended" only from the operator's perspective — git-wise it has completed.
...
8.5. **Program-defined completion**: re-read `program.md`'s `Campaign Completion` section (or equivalent — naming may vary across campaigns). If it prescribes campaign-specific finalization steps not already covered by steps 1-8 above (e.g., promoting an evolved profile back into the baseline profile, deleting the evolved profile, post-merge data migration, archival moves), execute them as a separate post-merge commit prefixed with `campaign:` (or `chore:` if a single mechanical step). **Suspended-campaign caveat**: if the campaign is *suspended* (resume pending a spec) rather than *terminated*, program-defined completion steps that are irreversible or destructive — profile promotion, evolved-profile deletion, archival moves — are likely premature; defer them to the true completion after resume, or surface the conflict to the user via the 1-3-1 rule. Mechanical, reversible steps may still run. Skip this step entirely if `program.md` has no such section or the section's steps were already executed as part of the squash-merge content. *(cwd: main repo root)*
