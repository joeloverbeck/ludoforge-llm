...
6. **Downstream consumers (blast radius)**: For types or interfaces the spec proposes to modify, grep for all import sites and usage points **in both source and test files**. Record the blast radius separately for source files (which need code changes) and test files (which need fixture/construction updates). Test migration scope is frequently underestimated in specs — flag it explicitly. For functions that currently throw errors, also grep for the error code/type across the codebase — errors propagate through call chains, so downstream catch sites may not directly import the modified function. **Catch-site categorization**: When the blast radius includes catch sites wrapping the modified function, categorize each by purpose: (a) probe/speculative execution, (b) graceful degradation (UI fallback on failure), (c) bug detection / re-throw. Different categories require different migration strategies — report category counts alongside the total.
7. **Existing implementations**: For each major proposed artifact (new types, new files, new patterns), search the codebase for existing implementations with similar names or functionality. Check whether the proposal duplicates existing infrastructure. This catches specs whose premise has been overtaken by prior work. Search using both exact names from the spec AND broader patterns (e.g., if the spec proposes `CompiledTokenFilter`, also search for `tryCompile*`, `*compiler.ts`, `Compiled*` to find related infrastructure that the spec may not reference). **Name collision check**: For each proposed new type, function, or export name, also check whether an *identically named* export already exists in the codebase — even if it serves a different purpose. Name collisions that would shadow or conflict with existing exports are Issues, not merely improvements. **Short-circuit**: If the initial exact-name search returns zero matches for the top-level artifact, note "no existing implementation found" and skip the broader pattern search. However, if the artifact *composes* existing named types (e.g., a result union wrapping an existing error class), the component type names still require collision checks — the short-circuit applies to the artifact as a whole, not its constituent types.
8. **Pattern consistency**: When a proposed type belongs to an established family (e.g., result union types, error types), verify it uses the same discriminant field names, variant shapes, and naming conventions as existing members of that family. Convention mismatches within a type family are Issues, not merely improvements.
9. **Quantitative claims**: If the spec states numeric metrics (file counts, function counts, workaround counts), verify them against codebase grep/glob results. Correct inaccurate numbers in the updated spec.

For specs with many references (>5 types/functions/paths), use one or more Explore agents (up to 3) to perform extraction and validation. Split across agents when concerns have different search patterns — e.g., structural validation (throw/catch sites, blast radius) vs. pattern analysis (existing types, convention consistency). Include the full spec content in each agent prompt so it can both identify references and validate them — summarizing references risks missing implied or embedded references that only appear in prose context. This is read-only — agent-based exploration is safe and significantly faster.

**Blast radius is mandatory**: The agent prompt MUST explicitly request blast radius analysis — grep for all import sites and consumer files of any type or interface the spec proposes to modify. This is the highest-value output from the Explore agent and must not be omitted. When catch sites are part of the blast radius, the agent prompt must also request catch-site categorization per Step 2 item 6.
...
**Plan mode**: Present the full findings report as text first (the structured report from Steps 2-4), then use `AskUserQuestion` for each question that requires user input. This gives the user the full context before being asked to decide. Inline text questions won't block in plan mode — `AskUserQuestion` is required for blocking interaction.
...
- **Architectural questions with multiple viable alternatives**: Provide a brief comparison of alternatives against FOUNDATIONS.md principles before presenting the recommendation. Format: a labeled list (`**Option A (name)**: F-number alignment summary`) followed by a `**Recommendation**` paragraph citing the decisive foundation(s).
...
2. **Present the diff summary** to the user as a numbered list: `N. **<section name>**: <one-line change description>`. When the spec proposes or modifies 3+ types, append a **Types** line after the list: `**New**: TypeA, TypeB | **Reused**: TypeC | **Removed from proposal**: TypeD` (omit empty categories).
