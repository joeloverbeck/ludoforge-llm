...
For specs with many references (>5 types/functions/paths), use an Explore agent to perform extraction and validation in a single pass. Provide the agent with either the full spec content or a comprehensive structured extraction of all references. If summarizing, ensure you capture every reference from every section — including those embedded in prose, code blocks, tables, and footnotes. The goal is completeness, not format. This is read-only — agent-based exploration is safe and significantly faster.
