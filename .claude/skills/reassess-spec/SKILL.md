...
A behavioral claim is "complex" if tracing it requires reading 3+ functions across 2+ files (e.g., control flow through a pipeline, failure mechanisms spanning multiple modules, multi-layer dispatch). If traceable within a single file or function, it's "simple."
