...
If 2+ findings target the same section heading or share a coherent theme (same failure mode, same user pain point), introduce them with a shared lead-in sentence (e.g., "Three Step 2 Confidence-handling findings:") before listing them. Each finding retains its own bullet, classification, and severity — the lead-in is presentation scaffolding only, not a merger.
