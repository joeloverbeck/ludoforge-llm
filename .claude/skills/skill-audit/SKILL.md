...
- **Follow-up implementation** — After the report is presented, the user may request implementation of specific suggestions. At that point, edit the target skill file directly — the "report only" guardrail applies only to the audit phase, not to user-directed follow-up. Always read the target skill file before starting edits (required by the Edit tool contract). When implementing multiple suggestions, process edits top-to-bottom within the file to avoid offset drift. When two suggestions target adjacent or overlapping text, combine them into a single Edit call. After making follow-up edits, always re-read at minimum each edited section plus 10 lines of surrounding context to verify edits don't conflict with each other or with unchanged skill content. A full-file re-read is required when edits touch adjacent sections, numbered lists, or shared structures. When edits span more than ~50 lines of the file, prefer a full re-read over reasoning about adjacency. Pay particular attention to numbered lists and sequential structures — insertions commonly break numbering, create duplicate headings, or split contiguous lists with stray text. If there was a session interruption between the audit report and implementation, re-read the target skill before editing to verify it hasn't been modified by another process. The user may run a subsequent `/skill-audit` on the same skill after edits to validate the changes — this is a normal audit-edit-reaudit workflow. If the system enforces plan mode for implementation, write a brief plan listing the edits top-to-bottom, then execute after approval. The plan is lightweight — it exists to satisfy the planning workflow, not because skill edits need architectural planning.
