...
**Append vs in-place update**: When the spec has no equivalent section, append a new `## Tickets` section at the end of the spec. When the spec already has a semantically-equivalent guidance section under a non-canonical heading (e.g., `## Follow-on tickets`, `## Follow-On Tickets`, `## Ticket Decomposition Guidance`), prefer in-place rename to `## Tickets` and replace the body with the canonical format above — preserve any pre-existing section number to maintain stable cross-references in the spec. Avoid creating a duplicate `## Tickets` section that fragments related decomposition content.
