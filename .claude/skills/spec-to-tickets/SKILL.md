...
- **Effort**: Small / Medium / Large, with the following rough calibration (heuristics, not hard thresholds):
  ...
  - **Large**: >8 files modified OR >250 LoC net OR a Foundation 14 atomic cut spanning the full deletion blast radius. Foundation 14 atomic cuts that are mechanically uniform remain acceptable as Large regardless of LoC count (per the existing Foundation 14 exception bullet in Step 3).
