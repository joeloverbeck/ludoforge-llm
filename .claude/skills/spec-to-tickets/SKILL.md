...
Include a 1-line scope description for each ticket either inside the Title column or as bullet text below the table — skip the bullets when the title column already conveys scope in one line (e.g., titles like `Phase 0 — Inventory ...` already encode scope without needing duplication). Deps in the summary table are abbreviated for readability (e.g., `001`, `None`). Ticket files use full backtick-quoted paths.
