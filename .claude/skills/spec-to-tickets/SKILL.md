...
`<spec-path>` accepts either an exact file path or a shell glob pattern. If the glob matches exactly one file, proceed with that file. If it matches multiple, list the matches and ask the user which to decompose (in auto mode, default to the highest-numbered match and surface the choice in the Step 4 summary). If zero matches, error and ask for a corrected path.
...
**Auto mode relaxes approval gates only** — process-discipline rules are unaffected: Step 5's single-message Write batching, Step 6's `check:ticket-deps` validation, Step 8's canonical back-link format, FOUNDATIONS alignment, ticket fidelity, codebase truth, and any other constraint listed in the Constraints section all apply identically under auto mode. The bypass is "skip the wait," not "skip the work."
...
- **Glob** for `tickets/<NAMESPACE>-*.md` AND `archive/tickets/<NAMESPACE>-*.md` — if any files with this namespace already exist (active or archived), warn the user and ask whether to overwrite, continue numbering from the next available number, or abort. Archived collisions should typically lead to a different namespace, since history reuse confuses traceability. **In auto mode**: default to "continue numbering from the next available number" for active-ticket collisions (safe — additive, reversible); default to "abort the run without writing tickets" for archived-ticket collisions (history reuse is a stronger signal that the namespace should change) and surface the collision in the Step 4 summary so the user can choose a different namespace before re-invoking
- **Dependency path resolution** (this sub-step only): For each spec dependency listed in the target spec's **Dependencies** field, verify whether it lives in `specs/` or `archive/specs/` and record the correct path for use in ticket Deps fields. If the Dependencies field indicates no dependencies (e.g., `None`, `None (...)`, or equivalent), skip this sub-step — the other validation sub-steps (file paths, types, namespace collision) still apply. Foundation principle references (e.g., `Foundation 4 (...)`, `F#14`) are conceptual citations, not files. Skip them in dep resolution; they belong in tickets' Architecture Check sections, not Deps fields.
...
Analyze the spec and identify discrete work units. If the spec includes a decomposition-guidance section under any of the following or similar heading patterns — `## Follow-On Tickets` (canonical reassess-spec output), `## Ticket Decomposition Guidance` (legacy name still used by older specs), `## Notes for ticket decomposition`, `## Decomposition Notes`, or equivalent — use it as the starting point for decomposition. Validate that the spec's suggested breakdown produces reviewable diffs and has correct dependency ordering, but do not ignore it in favor of a from-scratch decomposition. If the spec has no Decomposition Guidance section (common for scope-collapsed specs from `/reassess-spec`, where the reframe typically replaces earlier decomposition hints), use the Design (D*) and Testing Strategy (T*) sections as the primary decomposition anchors — each design section typically maps to one ticket, and each test attaches to the ticket introducing its subject.

**Namespace precedence**: If the spec's decomposition-guidance section lists ticket IDs under a namespace prefix that differs from the user-supplied `<NAMESPACE>` argument (e.g., the spec proposes `166CANPARAMREF-*` but the user invokes with `166CANPARREF`), the user argument is authoritative — write tickets under the user-supplied namespace, and note the divergence in the Step 7 summary so the user can verify the choice. This applies in auto mode as well, where there is no opportunity to confirm at the gate.
...
- **Cross-ticket budget tightening**: When a perf gate or similar threshold tightens across phase boundaries (e.g., 5500 ms → 3000 ms → 250 ms as successive optimizations land), tighten the budget in the ticket that delivers the corresponding speedup. The ticket that introduces the gate sets the initial budget; subsequent tickets that unlock measurable gains tighten it. This is distinct from atomic cuts (which are deletions) and from grouping (which merges deliverables) — it's a numerical update to a previously-introduced assertion. Acknowledge the next-step tighten in the introducing ticket's Out of Scope (e.g., "tightening to 3000 ms is owned by ticket 009").
...
- **Deferred-execution tickets**: Some specs require a ticket file to exist as a phase deliverable but defer its implementation work to a later phase (e.g., a post-rollout cleanup ticket whose execution waits until the rollout closes; a CI-restoration unwind whose work is gated on a downstream perf gate passing). These differ from Gate tickets in semantics: gate tickets may be closed-without-work via a descope path when the gate condition fails; deferred-execution tickets are unconditionally executed once their upstream dependencies close. Both use the same mechanism (backtick-quoted Deps + a "Gate condition" note in the Problem or What to Change section), but title and frame deferred-execution tickets so the deferred-not-conditional nature is clear (e.g., "CI restoration unwind (post-Phase-4)" rather than "CI restoration gate"). The Deps field cites both the upstream-that-blocks-execution AND any code-level reverts the ticket performs.
...
Include a 1-line scope description for each ticket either inside the Title column or as bullet text below the table — skip the bullets when the title column already conveys scope in one line (e.g., titles like `Phase 0 — Inventory ...` already encode scope without needing duplication). Deps in the summary table are abbreviated for readability (e.g., `001`, `None`, `spec` for the spec-file root-dep case where the root ticket cites `specs/<NNN>-<name>.md` per Step 5's Root tickets guidance). Ticket files use full backtick-quoted paths.
...
Waves express parallelism *opportunity*, not parallelism *requirement*. A ticket with fewer hard deps may be grouped with a later wave for thematic or review-ergonomic reasons (e.g., batching all test-authoring tickets together, or grouping witness tests after their shared fix tickets). When doing so, note the grouping rationale in a one-liner after the wave table (e.g., "007 hard-needs only 004 but is grouped with Wave 4 for reviewer convenience — all test tickets land together"). This keeps the wave plan honest about the actual dependency graph while documenting the ergonomic choice.
...
For each approved ticket, write a file to `tickets/<NAMESPACE>-<NNN>.md` using the **exact structure** from `tickets/_TEMPLATE.md`. Write all ticket files in parallel — each is an independent file create on a disjoint path with no shared state. Use multiple Write tool calls in a single message rather than sequential messages. Sequential writes cost noticeable wall-clock time on decompositions of 5+ tickets. **This holds for large decompositions too (15-20+ tickets)** — the harness handles parallel-tool-use messages of arbitrary size, and splitting across multiple messages adds wall-clock latency without benefit. Resist the urge to chunk Writes for perceived response-size "manageability"; the right answer remains one message regardless of ticket count.
...
- **Effort**: Small / Medium / Large, with the following rough calibration (heuristics, not hard thresholds):
  ...
  - **Medium**: 3–8 files modified OR introduces 1–2 new test files, <250 LoC net. **Conflict resolution**: when two Medium-vs-Large triggers disagree (e.g., file count says Medium but test-file count exceeds 2, or vice versa), default to Medium when the additional files are structurally uniform and individually small (each <50 LoC; e.g., a cluster of architectural-invariant tests sharing one fixture, or a set of golden-trace tests pinning sibling traces). Escalate to Large only when at least one of the additional files contains non-trivial new logic, new scaffolding, or new infrastructure that would itself dominate the review burden.
  ...
  - **Audit / documentation / investigation tickets** (where the primary work is cross-module reading and write-up rather than code changes): rate by read-scope and coordination complexity rather than LoC. Small for single-script or single-report work touching ~2 files; Medium when the audit spans 5+ source files or 3+ subsystems even when output LoC is well under 250; Large when the audit or documentation sweep crosses engine-runner boundaries or requires evidence capture plus synthesis. This calibration applies equally to evidence-only tickets (diagnostic script + checked-in report), classification/audit tickets (authoritative doc consuming prior evidence), and cross-module documentation sweeps — all have their effort determined by reading breadth, not writing volume.
...
- **Files to Touch**: Exact paths validated against the codebase (new or modify). **Audit-dependent tickets exception**: for tickets whose file set genuinely depends on a prior audit or classification ticket's findings (common in audit-driven chains where ticket N authors the authoritative classification and tickets N+1, N+2 act on its output), provide a `Likely surface` section with the tentative path list plus an explicit note that exact scope is refined against the upstream dependency's output. Downstream `/implement-ticket` will reassess the list against the upstream's findings during its Phase 2 reassessment. This pattern is distinct from laziness (failing to grep known-stable paths) — only invoke when the upstream audit ticket's classification itself defines the target set, and keep the `Likely surface` list as concrete as possible with a one-line rationale per path. **Migration / removal tickets exception**: When the ticket removes a bucket, feature, or symbol with a known cross-codebase blast radius (typical of Foundation 14 atomic cuts — e.g., removing a deprecated bucket from `AGENT_POLICY_LIBRARY_BUCKETS` plus all source/test references), enumerate the actual affected file list via grep at ticket-authoring time rather than deferring to implementation. The reassessment phase typically characterizes blast radius as a count ("~20 test files") rather than a path list; do not propagate the count as-is — run `grep -rln '<symbol>' packages/<pkg>/src packages/<pkg>/test data/games` (or equivalent) at ticket-authoring time and list the actual paths. The implementation phase shouldn't need to rediscover the inventory the reassessment already characterized; pasting paths now costs one grep, deferring costs `/implement-ticket` a structure rediscovery the depth-of-read rule (last bullet of Step 3) explicitly warns against. The Audit-dependent exception above applies only when an upstream audit/classification ticket's findings define the target set; migration tickets whose target set is already greppable today do NOT qualify for the `Likely surface` deferral.
...
If the spec does not already have a section listing the actual generated ticket IDs (as distinct from a decomposition *guidance* section with suggested prefixes), offer to append or update one with the generated ticket IDs and their titles. This aids traceability when multiple specs are active. If the user declines, skip. May be combined with Step 7 in a single conversational response — the back-link append uses an `Edit` tool call against the spec file, while the Step 7 summary is conversational text. Combine the *presentation*, not the *tool calls* — they're separate operations. **In auto mode**: append the back-link proactively in the same response rather than waiting for confirmation — an additive `## Tickets` section is reversible, and auto mode explicitly prefers action over interruption.

**Canonical back-link format**: Use a `## Tickets` heading, a one-line provenance prefix `Decomposed via \`/spec-to-tickets\` on YYYY-MM-DD:`, and a bulleted list with one entry per generated ticket of the form `` - [`tickets/<NS>-NNN.md`](../tickets/<NS>-NNN.md) — <Title> (covers <spec-section>) ``. The `(covers <spec-section>)` annotation references the I-section, D-section, or other decomposition anchor the ticket implements; omit it when the spec has no such anchors OR when the ticket title already embeds the anchor (e.g., `Phase 0 — ...` titles for specs whose decomposition anchors are §4 Phases). The back-link lists *generated* tickets, not suggested IDs from any decomposition-guidance section.

**Append vs in-place update**: When the spec has no equivalent section, append a new `## Tickets` section at the end of the spec. When the spec already has a semantically-equivalent guidance section under a non-canonical heading (e.g., `## Follow-on tickets`, `## Follow-On Tickets`, `## Ticket Decomposition Guidance`, `## Notes for ticket decomposition`, `## Decomposition Notes`, or equivalent), distinguish two sub-cases based on the section's body:

- **Body is a list of draft / suggested ticket IDs** (e.g., bulleted candidates like `- FOO-001 — probe runner`): in-place rename to `## Tickets` and replace the body with the canonical format above — the draft ID list is superseded by the generated IDs. Preserve any pre-existing section number to maintain stable cross-references.
- **Body is decomposition advice in prose** (categories, count hints, ordering recommendations — e.g., "Phase 0 should decompose into: probe runner, assertion library, ..."): preserve the existing section unchanged and append a new `## Tickets` section after it. The advice is historical provenance that survives as context for future reviewers; the back-link captures the generated tickets without erasing the original guidance.

Avoid creating a duplicate `## Tickets` section that fragments related decomposition content.
