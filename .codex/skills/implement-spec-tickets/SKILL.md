...
If a profiling or baseline harness uses nested Node child processes and fails with impossible parser output, empty stdout/stderr, `EPERM`, or missing child-process output under the sandbox, classify sandbox interference before changing code. Rerun the exact same command outside the sandbox with user approval when the command is important to the ticket proof. Only patch implementation or harness code after the unsandboxed rerun proves the failure is real.
...
5. Validate `.codex/run-state/implement-spec-tickets.json` if it changed: live paths exist or are intentionally archived/final, queued paths exist, `last_work_commit` is a full reachable SHA or `"none"`, `last_state_commit` is a reachable SHA, the same SHA as `last_work_commit`, `"self"`, or `"none"`, and `dirty_state` matches the worktree classification. Prefer `node .codex/skills/implement-spec-tickets/scripts/validate-state.mjs` when available. `dirty_state` must use one of the documented forms (`"clean"`, `unrelated_untracked: ...`, `unrelated_dirty: ...`, `owned_dirty: ...`, or `mixed_dirty: ...`), unless this skill has been updated to document a new form first. If the only live dirty path is `.codex/run-state/implement-spec-tickets.json` and the file intentionally records the expected post-commit state as `dirty_state: "clean"`, run the validator with `--allow-only-state-file-dirty` before the state-only commit; after the state commit, rerun the validator without that flag plus `git status --short`. For any other state-file-only follow-up commit, validate normally before staging.
