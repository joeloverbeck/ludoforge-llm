...
When a proof command appears hung and normal session input cannot stop it cleanly, perform narrow cleanup before continuing: poll the session once, inspect matching processes with a precise pattern (`pgrep -af` or equivalent), terminate only the command/process group for the hung proof lane, confirm no matching child processes remain, and record the lane as `harness-noisy / not final-confirmed` or another truthful non-green classification. Do not leave runaway proof processes active, and do not treat a manually terminated lane as either passing or a functional red assertion without direct evidence.
