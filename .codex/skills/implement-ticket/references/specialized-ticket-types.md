...
When a ticket, reviewer note, or spec adds qualitative materiality language such as "significant reduction", "meaningful improvement", or "not just a tiny follow-up", make that judgment explicit before closeout. Classify the final measured delta as `material`, `minor`, or `not demonstrated`; record the baseline/current numbers and rationale; and if the measured gate remains red with only a minor or undemonstrated reduction, use `BLOCKED`, `PARTIAL`, or `1-3-1` unless the active ticket already authorizes successor-based completion on the reduced-but-red result.
