...
When the decisive measurement is a same-code rerun that refreshes a baseline or report without retaining a runtime change, label timing differences as current-run evidence rather than candidate improvement. The report or ticket should say whether the numbers are `fresh same-code rerun`, `candidate delta`, or `mixed / unclassified drift`, and should avoid causal wording unless an A/B or before/after code comparison proves it.
...
When profiling or benchmark tickets add timing buckets that will feed materiality, share-of-wall, residual, or top-axis decisions, classify each new bucket before the decisive witness as `exclusive/additive`, `inclusive parent`, or `nested child`. Do not sum inclusive parents with nested children as if they were disjoint. The report, CSV headers, ticket outcome, or successor handoff must preserve the classification in labels and residual math so readers can tell whether a row is an additive cost, a containing umbrella, or a diagnostic child slice. If a smoke witness shows the bucket shares can exceed 100% of the measured axis because of nesting, fix the labels/math or keep the result diagnostic until a same-output-shape witness confirms the corrected report.
