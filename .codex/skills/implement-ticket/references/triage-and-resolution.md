...
- Existing substrate for a ticket-named new primitive: when the ticket asks for a new helper, plan, compiler, cache, or runtime primitive but live code already has an analogous substrate, classify the deliverable before coding as `new primitive still required`, `wire existing primitive`, or `already satisfied / verified-no-edit`. If this changes the explicit deliverable noun, artifact, acceptance witness, or public contract, stop for `1-3-1` unless the ticket already authorizes that substitution. Record the chosen mapping in working notes and the active ticket outcome.
