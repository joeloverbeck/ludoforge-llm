...
- For Turbo or other cached package/workspace lanes, decide in advance how a cache replay will count. If the final command returns from cache after this session changed files, classify it as `cache-covered`, `cache-hit supplemental`, or `cache-hit proof pending`. Load `references/verification-acceptance-proof.md` when the cached lane is primary proof for a touched surface or coverage is uncertain; if direct focused/no-cache lanes already prove the changed surface and the cache hit is plainly supplemental, record `cache-hit supplemental` without loading the fuller reference.
  - For no-code or report-only tickets where this session changed only tickets, specs, docs, reports, CSVs, or other non-executable evidence artifacts, cached broad Turbo lanes can be classified as `cache-covered` or `cache-hit supplemental` without the fuller reference when the decisive evidence command is fresh and the active ticket records that no source, test, schema, generated runtime artifact, or package manifest changed. Pair that classification with graph/status integrity and artifact hygiene instead of treating the cached broad lanes as the primary proof.
...
For multi-step accelerator routes such as `chooseNStep` continuations, do not stop at the first accelerated command. Enumerate every command class the accelerated path can reach after first activation, including terminal or confirmation steps, and prove each is handled through the accelerated materialization path or is explicitly classified as unsupported/fallback. A first-step activation counter is not enough if a later continuation silently falls back while the ticket claims route support.

When unit tests assemble synthetic microturns, state patches, ABI buffers, or decision-stack fragments by hand, classify what they prove. Synthetic fixtures can prove lowering, encoding, decoding, and fail-closed shape, but they are not behavior proof when the state and published microturn could not arise together through the public runtime seam. Pair synthetic fixture proof with a public-seam materialization, route-activation, or parity witness before citing user-visible behavior.
...
For newly authored test files, preflight repo-specific metadata before the broad package lane. Inspect nearby tests and any guard that validates file headers, tags, ownership markers, class markers, fixture registrations, or lane manifests. Reuse an existing accepted value from the adjacent corpus instead of inventing a new marker label. A marker/header failure is test setup fallout, not a product red witness; fix it before citing the broad lane.
...
7. **Long-running commands**: Some ticket-required commands may run for minutes with sparse output. Treat that as normal when consistent with repo history; keep running and provide periodic progress updates.
   ...
   - For very verbose Turbo or workspace lanes, use a final-summary capture by default when the lane is ticket-final, broad acceptance proof, or likely to exceed terminal output limits: send output to a temp log or tee-like capture, then inspect the final summary/tail and `rg` for ticket-owned test file names or package summaries. For exploratory or clearly small lanes, a normal terminal run is fine. Cite the command exit status plus the extracted summary; keep direct focused reruns as the primary proof when they exercise the owned boundary more clearly than a broad log.
     - Template: `pnpm turbo test > /tmp/<ticket-id>-turbo-test.log 2>&1`, then inspect the exit status, `tail -n 80 /tmp/<ticket-id>-turbo-test.log`, and `rg -n 'Tasks:|failed|ERROR|<ticket-owned-test-or-package>' /tmp/<ticket-id>-turbo-test.log`. Substitute the ticket-named lane and log stem as needed. If the summary is missing or the exit failed, inspect the log before classifying the result.
