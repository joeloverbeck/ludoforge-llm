...
This reference is the canonical compact checklist for normal Codex runs. The five hard-stop fields above are the output shape; the labels below are expanded aliases to help fill those fields. If the main `SKILL.md` adds fields for a specific ticket type, use the union of this list and the main skill's triggered fields rather than treating this section as a reason to omit them.
...
- `expected generated fallout`: schema artifacts, goldens, compiled JSON, or `none`; if editing `schemas-core.ts`, serialized trace/result unions, generated-schema-bearing types, or other compiled public schema surfaces, default to `schema artifacts likely` until `schema:artifacts:check` proves otherwise. For generated reports, CSVs, traces, or benchmark artifacts, smoke one cheap output when practical before the expensive/full run and inspect headers, mode labels, reproducer command text, and filename shape.
- `verification substitutions`: any repo-valid replacement command or required flag/output-path correction
- `command ledger`: when any literal ticket command/shorthand is stale, split, replaced, subsumed, or intentionally not run, record `ticket section | literal command/shorthand | ran directly/subsumed/split/replaced/not run | final citation` before final proof; do not leave the mapping only in prose. For generated reports, measured artifacts, or reproducer commands, include every env var, cwd assumption, flag, and output label that changes the artifact or metric; a command that omits an active env flag is not a faithful reproducer.
- `AGENTS canonical lane reconciliation`: when `AGENTS.md` lists broader root lanes than the ticket requires, record `AGENTS canonical lane | required by ticket? | ran/subsumed/not applicable | rationale` before terminal closeout. This is mandatory when a root lane is omitted, split into package-local commands, replaced by a focused witness, or cited only as cached/supplemental evidence.
- `reference guidance loaded`: triggered references actually loaded for the current ticket, such as `working-notes`, `schema-and-migration`, `verification`, `verification-acceptance-proof`, or `verification-evidence`; if a normally triggered reference is skipped, record `not loaded + why`
  ...
  - Include `verification-evidence.md required? yes/no`. Use `yes` for trace-heavy investigation tickets, campaign/manual validation evidence, saved traces, harness summaries, decision-gap inspection, campaign metrics, generated artifact triage, or negative production-surface claims.
...
- `output contention / sequencing`: classify each planned proof lane that consumes or rewrites `dist`, schemas, goldens, compiled JSON, or another generated tree as `parallel-safe` or `serial-only`; record the producer step and any rerun required if a later lane cleans/rebuilds that output
  - For explicit byte-identical build-output proof, prefer this low-noise recipe: build once, write a sorted SHA-256 manifest for the generated tree to `/tmp`, build again, write a second sorted manifest, `diff -u` the manifests, and cite the diff result plus manifest file count. Do not print the full manifest into the conversation or ticket unless the diff fails and the differing paths are needed.
...
- `negative production-surface witness`: when an investigation/reset ticket may close by proving no usable production witness exists, plan the absence proof explicitly: `source inventory`, `live trace/probe`, `nearest plausible production candidate`, `synthetic-only evidence`, `generic defect? yes/no`, and `successor owner`
- `verdict/status vocabulary propagation`: when a ticket adds, removes, renames, or reinterprets prose labels such as verdicts, status branches, decision rows, outcome labels, warning names, unsupported reasons, or other enum-like vocabulary, treat those labels as a shared contract even if they are not typed in code. List changed labels, sweep active specs/reports/tickets and direct dependent siblings that consume them, patch stale consumer prose, and run the ticket-dependency or markdown-integrity lane when dependency/status graph edges or active sibling state changed. For checked-in measured reports, reread the report after late label/spec/sibling updates and patch stale future-tense or pre-update wording before closeout.
...
- `source file size risk`: optional; include when a named source file is already near/over repo guidance and active work will add logic there, or when the ticket creates a substantial new source file likely to carry most of the implementation. Use `source-size risk check` for cheap under-cap sanity counts; use `source-size hard-gate ledger` only when a file crosses the cap, ends over the cap because of active growth, was already over cap and grew, or needs explicit deferral. For profiling/investigation tickets, update this field when profiling selects an unlisted implementation file that may be near/over guidance. When retaining active growth in a near-cap or over-guidance source file, plan the closeout ledger in this exact shape: `path | before lines | after lines | crossed cap? | active growth | extraction/defer rationale | successor if any`.
  ...
  - Before terminal status, make this a mechanical all-grown-source sweep rather than a memory check. Use `git diff --numstat` or an equivalent diff inventory plus `wc -l` for every touched source file whose active growth is `>=100` lines or whose final size is `>=600` lines. Add a compact table for each triggered file: `path | before lines | after lines | active growth | crossed cap? | ledger status`. After shared-contract, schema, generated-artifact, or fixture fallout is discovered, rerun this sweep for every source file that grew in the final diff, not just source files named by the draft ticket. Add any new near-cap, crossed-cap, or preexisting-oversize-plus-active-growth entries to the closeout ledger before terminal status.
...
Post-terminal proof transcription exception: if a post-status integrity lane is required because terminal status itself changed the graph, record that lane in the active ticket only as a clerical proof-ledger/outcome edit. Do not change scope, status, deps, or sibling ownership in that transcription. Rerun `git diff --check`; rerun the graph checker only when the transcription itself changes graph-affecting claims. Otherwise, cite the post-status lane in the final response without reopening the ticket.

Post-terminal metadata repair: if the final sweep discovers a missing source-size, touched-file, dirty-state, command-ledger, or no-invalidation transcription after terminal status, first classify whether the omission changes scope, acceptance boundaries, command semantics, graph/status/dependency facts, touched-file ownership, proof claims, or follow-up ownership. If it changes any of those, reconcile status and rerun the narrowest affected proof or integrity lane. If it is metadata-only, patch only that ledger text, rerun `git diff --check`, rerun the graph checker only when graph-affecting text changed, and record the repair as non-invalidating in the ticket outcome or final closeout.

If you resume from context compaction, interruption, or a long handoff summary, do not rely on a summary sentence that says the checkpoint happened unless the full ticket-named deliverables ledger is visible. For resumed closeout, "full current state" means the visible context already names the active ticket/status, remaining proof rows, untracked artifacts, in-flight command/session, post-proof edit class, integrity lane, and next status/handoff. If any of those are missing or stale, reconstruct the ledger from the active ticket's explicit `What to Change`, `Files to Touch`, named artifacts/tests, `git diff --name-only`, and `git status --short` before any further file edit or terminal closeout. Re-emit the reconstructed ledger when it changes the owned boundary, proof plan, or closeout status.
