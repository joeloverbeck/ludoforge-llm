...
I've chosen Fire in the Lake, the COIN game, to implement as a YAML Game Spec precisely because it's an extremely-complex game with interlocking mechanics, that forces humans to stop their turns to check out byzantine rules. If we can codify this game and make it play as it does in reality, this app will be extremely strong.

Our goals, based on the game rules provided:

1) Figure out the discrete mechanics to implement.
2) Figure out if those discrete mechanics are already implementable.
3) If not implementable, figure out how they could be. We don't want hacks or patches into our code: we're very early, we don't need backwards compatibility or legacy code. If some part of the rules we need to implement suggests we need a restructuring of some part of the code, then restructure, fix existing tests, create comprehensive new tests.
4) If you come up with any bugs throughout the process of implementing these rules, fix it through TDD.
5) If you can come up with improvements, do so.

Note: Fire in the Lake has very complex base rules, but much of the varying complexity is in its big deck of cards. I'll only show a couple of those cards to implement. This spec intends for us to codify and make sure that the base rules are playable as expected, and prove it with tests through the simulation runner, play logs, etc.

## Foundation scope and constraints

This brainstorming document now targets a **foundation slice** of Fire in the Lake that is large enough to prove model fidelity, but intentionally not the full product.

In scope for this foundation:
- Core map/space model (Province, City, LoC, foreign-country semantics, adjacency representation).
- Core pieces, markers, and tracks (Support/Opposition, Control, Resources/Aid/Patronage, Trail, Eligibility).
- Turn framework (Event card sequence, Eligibility/Ineligibility, Limited Ops, Monsoon limits, Coup round phases).
- Faction Operations and Special Activities (US, ARVN, NVA, VC) using the base rules listed below.
- Event framework behavior (unshaded/shaded execution, capabilities/momentum semantics, free Ops semantics).
- A thin initial event-card slice: Card 82 (Domino Theory) and Card 27 (Phoenix Program).
- Deterministic simulation and tests proving rules behavior via traces.

Out of scope for this foundation:
- Full 130-card deck implementation.
- Non-player flowcharts and bots from section 8.0 of the rulebook.
- Optional/advanced modules not required to execute the base campaign loop.
- Full production-perfect historical adjacency validation from artwork extraction (we can stage this with a curated dataset first).

## Baseline assumptions for implementation

- `GameSpecDoc`/`GameDef` may require targeted extension when rules cannot be represented cleanly. Favor principled schema/runtime evolution over one-off hacks.
- If a rule is ambiguous in this transcription, implementation specs must choose one deterministic interpretation and document it.
- All random branches must be seed-driven and trace-visible.
- Foundation acceptance is not "all FITL rules forever", but: setup -> several event cards + coup -> valid victory check, with reproducible traces.
...
Adjacency and value data should be delivered as an explicit machine-readable map dataset in this foundation (not inferred ad hoc at runtime). If some adjacencies cannot yet be confirmed from source material, mark them as provisional in data and in tests so they can be replaced safely in a later follow-up without changing engine semantics.

## Two cards

These two cards are from the deck, to see if we could implement them as well. In a different spec, we'll handle the rest of the cards.

### Card one

Card features:
- Period Event (Set Up Option): 1965
- Faction Order (shows the four factions): ARVN, VC, US, NVA
- Card number: 82
- Image: an american pointing at a map on a wall
- Title: Domino Theory
...
- Lasting effects if any (5.3): none
...
- Faction Order (shows the four factions): US, VC, ARVN, NVA
- Card number: 27
- Image: dead young Vietnamese person on the ground
