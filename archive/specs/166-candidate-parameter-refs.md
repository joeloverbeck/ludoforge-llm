...
declared `params: []`, but `packages/engine/src/kernel/legal-moves.ts:1273-1325` (`enumerateCurrentEventMoves`) emits candidates with `params: { eventCardId, eventDeckId, side, branch? }`. Phase 5 now declares the always-present `eventCardId`, `eventDeckId`, and `side` params on the FITL `event` action, so those refs resolve through `catalog.candidateParamDefs`. The optional `branch` param remains undeclared under the current required-param contract because branchless event moves are legal.
...
- **No new GameSpecDoc action-params field shape.** The existing `params: [{name, domain}]` shape remains the action-param contract. Ticket `166CANPARREF-002` adds a small generic `booleans` options query so boolean candidate params can be declared through that same `domain` slot. No per-game schema files, no `domain.kind: enum`, and no `valuesFrom: dataAsset` invention are introduced by the parser-validation slice.
...
preferEventBranch:
  ...
  value:
    boolToNumber:
      eq:
        - ref:
            candidate.params.optionalBranch:
...
Ready resolutions are proven by the existing score contribution path plus absence from `unknownCandidateParamRefs`; the live trace contract does not add a parallel ready-ref bucket. This keeps the per-candidate trace shape compact while preserving explicit provenance for unavailable candidate-param refs.
...
| 1 | Parser acceptance + scope validation | `candidate.params.<paramName>` lowers to the new compiled ref; singular form continues to reject; microturn-scope reference rejected; generic boolean action-param domains compile through `query: booleans`; round-trip compile test asserts the full ref shape including `onMissing` and optional `appliesToActions` | M |
...
| 4 | Trace plumbing + analytics | `unknownCandidateParamRefs` propagates through `microturn-option-eval.ts` and `microturn-option-evaluator.ts` aggregation; `candidateParamFallbackFired` recorded; ready candidate-param behavior is proven through score contribution plus absence from `unknownCandidateParamRefs` | S |
| 5 | FITL action declaration update | `data/games/fire-in-the-lake/30-rules-actions.md` `event` action declares the always-present params `eventCardId`, `eventDeckId`, and `side`; optional `branch` remains undeclared under the current required-param contract. Cross-action `candidateParamDefsEqual` check passes. A profile fixture exercising `avoidShadedEvent` over a 1-seed FITL trace produces the expected `contribution: -800` for shaded candidates and `0` for unshaded. | M |
...
9. **`candidate-params-determinism.test.ts`** — Replay a move twice; assert byte-identical `unknownCandidateParamRefs` order, `candidateParamFallbackFired` order/count, and score contribution values. Foundation #8.
...
12. **`candidate-params-toy-same-action-tracing.test.ts`** — Two legal candidates share `actionId: chooseMode` but differ by `params.mode ∈ {A, B}`. A consideration penalizes `mode=B`. Assert the ready candidates score through the expected contributions (`A` contributes 0, `B` contributes `-<weight>`) and both candidates' `unknownCandidateParamRefs` are empty.

13. **`candidate-params-fitl-seed-1001-side.test.ts`** — With `avoidShadedEvent` active in a FITL ARVN profile, the card-78 shaded candidate at seed 1001 turn N records `contribution: -800`, the unshaded candidate records `0`, and both candidates have empty `unknownCandidateParamRefs`. Pinned trace under `packages/engine/test/policy-profile-quality/candidate-params-fitl-witness/`.

14. **Optional-param missing fallback witness** — A synthetic action candidate with a declared optional-style param omitted from `move.params` resolves through `onMissing: { kind: constant, value: __absent__ }` and does not route through `unknownCandidateParamRefs` because the constant fallback fired. FITL `event.branch` is not the Phase 5 witness because `branch` remains undeclared until a future optional-param or microturn-lowering design owns it.
...
| #9 (Replay, Telemetry, and Auditability) | Reinforced — `unknownCandidateParamRefs` map and `candidateParamFallbackFired` channel expose unavailable/fallback provenance; ready behavior remains auditable through score contribution and absence from `unknownCandidateParamRefs`. |
...
- `data/games/fire-in-the-lake/30-rules-actions.md:160` — FITL `event` action declaration (Phase 5 — declare always-present `eventCardId`, `eventDeckId`, and `side`; leave optional `branch` undeclared under the current required-param contract)
...
4. **FITL `event` action `branch` domain**: FITL events declare branches as plain string ids inside each card's `unshaded.branches[]` / `shaded.branches[]`, but non-branching event sides omit `branch` entirely. Phase 5 selected option (c): leave `branch` undeclared on the `event` action under the current required-param contract. The live GameSpecDoc grammar does not support `valuesFrom: { dataAsset: ... }`, and declared action params are apply-time required, so declaring `branch` now would reject legal branchless event moves. `candidate.params.branch` remains unavailable unless a future optional-param or microturn-lowering design owns that surface.
...
| New typed `domain` grammar with `kind: enum, valuesFrom: { dataAsset }` and explicit `presence: required | optional` | **Rejected as proposed; adopted in spirit** (§3) | Codebase verification: the existing `params: [{name, domain}]` shape with `domain.query: enums, values: [...]` (as `pivotalEvent.eventCardId` already uses) is sufficient. No new grammar required. `presence` is handled by the candidate-emitter naturally (the param appears or doesn't on each emitted candidate) plus the ref-local `onMissing` policy; no schema-level `presence: required` field is needed. Open question 4 records why FITL `event.branch` remains undeclared until a future optional-param or microturn-lowering design owns it. |
...
| Trace bucket `candidateParamRefs[]` for ready resolutions + separate provenance category | **Rejected after live-code alignment** (§4.5) | The live trace contract has no generic ready-ref channel equivalent to `considerations[].refs[]`. Adding one here would create a parallel serialized trace structure. The accepted shape adds ONE new map (`unknownCandidateParamRefs`) mirroring `unknownPreviewRefs` / `unknownLookupRefs`; ready candidate-param behavior is proven by score contribution plus absence from the unknown map. |
...
| FITL action declaration fix: declare `eventCardId`, `eventDeckId`, and `side` on `event`; confirm `pivotalEvent.eventCardId` already canonical; leave optional `branch` undeclared under the current required-param contract | **Adopted with Phase 5 boundary correction** (§7 Phase 5, §11.4) | Codebase verification: `event` declares `params: []` while `legal-moves.ts:1303-1325` always emits `eventCardId`, `eventDeckId`, and `side`, and emits `branch` only for branching event sides. Phase 5 closes the always-present param mismatch without inventing `valuesFrom: dataAsset` grammar or making branchless events illegal. `pivotalEvent.eventCardId` declaration is already canonical and unchanged. |
