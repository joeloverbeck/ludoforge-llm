...
**Deps**: `archive/specs/150-fitl-policy-vm-wasm-port.md`, `archive/tickets/150FITLWASM-021.md`
...
2026-05-04 implementation landed a generic bounded dynamic Zobrist feature-key
cache while preserving the active WASM score-row and preview-state routes.

- `packages/engine/src/kernel/zobrist.ts` now keeps a per-`ZobristTable`
  `WeakMap`-owned bounded cache for dynamic feature keys such as `turnCount`,
  `decisionStackFrame`, `lastingEffect`, `nextFrameId`, and `nextTurnId`.
  These keys still do not enter the run-local structural `table.keyCache`, so
  the existing bounded-runtime cache boundary remains intact.
- `packages/engine/test/unit/zobrist-table.test.ts` proves repeated dynamic
  feature keys are deterministic, avoid a second uncached FNV computation, and
  do not add entries to `table.keyCache`.

Measured result:

- Baseline after fresh build:
  `timeout 180 node --cpu-prof --cpu-prof-dir=/tmp/ludoforge-150fitlwasm022-baseline-profile packages/engine/scripts/profile-fitl-preview-drive.mjs --seed 42 --maxTurns 1 --profilesAll --perCard --profileBuckets --label spec150-wasm-022-baseline-cpu`
  — RED, per-card `elapsedMs=2608.58`,
...
- Post-change same-seam profile:
  `timeout 180 node --cpu-prof --cpu-prof-dir=/tmp/ludoforge-150fitlwasm022-dynamic-key-profile packages/engine/scripts/profile-fitl-preview-drive.mjs --seed 42 --maxTurns 1 --profilesAll --perCard --profileBuckets --label spec150-wasm-full-hash-query-eval-residual-perf`
  — RED, per-card `elapsedMs=2539.8`,
  `wasmScoreRowUnsupportedCount=0`,
  `wasmPreviewCandidateFeatureRowUnsupportedCount=0`,
  `zobristKeyCacheMissCount=3717`,
  `zobristKeyCacheHitCount=189243`, `zobristKeyUncachedCount=334`.
- Post-change CPU parser evidence still shows residual owners in `fnv1a64`
  under token-placement hash updates, `resolveRef`, `evalCondition`,
  `evalValue`, `evalQuery`, `encodePolicyBytecodeInput`, and token-index
  refresh.

Retained candidate classification:

- `root-cause counter improved`: repeated dynamic feature-key hashing now hits
  the per-table bounded dynamic cache. The same-seam probe reduced
  `zobristKeyUncachedCount` from `1391` to `334`.
- `wall-clock gate still red`: the decisive same-seam probe remained around
  `2.54 s` versus the `<=250 ms` target, so no perf gate test was added and
  `149FITLEVNUMVM-016` remains blocked.

Rejected candidate ledger:

- `candidate`: microturn decision-state full-hash delta replacement.
- `correctness proof`: `timeout 90 pnpm -F @ludoforge/engine exec node --test dist/test/determinism/zobrist-incremental-parity-fitl-seed-42.test.js`.
- `measurement`: not retained; the focused correctness witness failed with
  `HASH_DRIFT` during the FITL seed-42 parity run.
- `decision`: reverted before closeout because the candidate did not preserve
  Foundation 8 hash determinism.

Created successor `tickets/150FITLWASM-023.md` for the next non-overlapping
owner: residual query/eval/reference-resolution and token-placement hash
closure. Tickets `149FITLEVNUMVM-016` and `149FITLEVNUMVM-022` remain blocked
until that or a later successor makes the `<=250 ms` gate truthful.

Schema/artifact fallout: none. The change is an internal per-runtime Zobrist
feature-key memoization path and a focused unit test; no serialized schema,
ABI, golden, or compiled GameDef artifact changed.

Final proof:

- `pnpm -F @ludoforge/engine build` — PASS.
- `timeout 90 pnpm -F @ludoforge/engine exec node --test dist/test/unit/zobrist-table.test.js` — PASS.
- `timeout 90 pnpm -F @ludoforge/engine exec node --test dist/test/determinism/zobrist-incremental-parity-fitl-seed-42.test.js` — PASS.
- `timeout 90 pnpm -F @ludoforge/engine exec node --test dist/test/unit/agents/policy-preview-driver.test.js` — PASS.
- `timeout 180 node packages/engine/scripts/profile-fitl-preview-drive.mjs --seed 42 --maxTurns 1 --profilesAll --perCard --profileBuckets --label spec150-wasm-full-hash-query-eval-residual-perf` — RED by threshold, active route clean, per-card `elapsedMs=2539.8` versus `<=250`.
- `pnpm run check:ticket-deps` — PASS after creating `tickets/150FITLWASM-023.md` and repointing dependent tickets.

No-invalidation note: the post-profile ticket/spec/dependency edits transcribe
the measured red result and successor ownership only; they did not change code,
command semantics, thresholds, or acceptance boundaries.
