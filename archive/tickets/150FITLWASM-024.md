...
**Deps**: `archive/specs/150-fitl-policy-vm-wasm-port.md`, `archive/tickets/150FITLWASM-023.md`
...
2026-05-04 implementation landed run-local initial full-hash Zobrist table cache
reuse while preserving the active WASM score-row and preview-state routes.

- `packages/engine/src/kernel/initial-state.ts` now uses the supplied
  run-local `GameDefRuntime.zobristTable` for initial full-hash computation
  instead of creating a throwaway table. Canonical hash values stay unchanged,
  while initial token-placement and structural feature keys remain available to
  later same-run hash verification and reconciliation.
- `packages/engine/test/unit/kernel/zobrist-incremental-tokens.test.ts` now
  proves initial-state hashing with a supplied runtime still matches canonical
  full recomputation, keeps `_runningHash` equal to `stateHash`, and populates
  the supplied run-local table cache.

Measured result:

- Baseline after fresh build:
  `timeout 180 node --cpu-prof --cpu-prof-dir=/tmp/ludoforge-150fitlwasm024-baseline-profile packages/engine/scripts/profile-fitl-preview-drive.mjs --seed 42 --maxTurns 1 --profilesAll --perCard --profileBuckets --label spec150-wasm-024-baseline-cpu`
  — RED, per-card `elapsedMs=2441.04`,
...
- Post-change same-seam profile:
  ...
  — RED, per-card `elapsedMs=2467.29`,
  `wasmScoreRowUnsupportedCount=0`,
  `wasmPreviewCandidateFeatureRowUnsupportedCount=0`,
  `zobristKeyCacheMissCount=2319`,
  `zobristKeyCacheHitCount=188266`, `zobristKeyUncachedCount=333`.
- Post-change CPU parser evidence after the retained cache-reuse slice still
  shows residual owners in initial-state/full-hash token-placement hashing,
  `resolveRef`, `evalCondition`, `evalValue`, `evalQuery`,
  `encodePolicyBytecodeInput`, and token-index refresh/build work.

Retained candidate classification:

- `root-cause counter improved`: initial full-hash key generation now seeds the
  supplied run-local runtime Zobrist table instead of a discarded local table.
  The same-seam gate reduced `zobristKeyCacheMissCount` from `2837` to `2319`
  and increased `zobristKeyCacheHitCount` from `187747` to `188266`.
- `wall-clock variance/unproven`: the decisive same-seam profile moved from the
  diagnostic baseline `2441.04 ms` to final `2467.29 ms`, so this ticket does
  not claim a wall-clock improvement.
- `wall-clock gate still red`: the decisive same-seam profile remained around
  `2.47 s` versus the `<=250 ms` target, so no perf gate test was added and
  `149FITLEVNUMVM-016` remains blocked.

Rejected candidate ledger:

- `token-placement FNV prefix-state helper`: exact-value proof and build passed,
  but same-seam probe regressed to per-card `elapsedMs=2558.03` and CPU evidence
  showed higher FNV self-time due to the added helper shape. The candidate was
  removed before final proof.

Created successor `tickets/150FITLWASM-025.md` for the next non-overlapping
owner: query/eval/reference-resolution, initial/full-hash token-placement,
token-index, and residual encoding closure. Tickets `149FITLEVNUMVM-016` and
`149FITLEVNUMVM-022` remain blocked until that or a later successor makes the
`<=250 ms` gate truthful.

Post-review cleanup:

- `tickets/150FITLWASM-025.md` was amended with the retained CPU profile
  artifact path, parser command, baseline/current metric, top residual owners,
  and non-overlap rationale. This was handoff evidence only; it did not change
  code, command semantics, thresholds, dependency ownership, or the active-route
  acceptance boundary.

Verification:

- `pnpm -F @ludoforge/engine build` — PASS.
- `timeout 90 pnpm -F @ludoforge/engine exec node --test dist/test/unit/kernel/zobrist-incremental-tokens.test.js` — PASS.
- `timeout 90 pnpm -F @ludoforge/engine exec node --test dist/test/unit/agents/policy-preview-driver.test.js` — PASS.
- `timeout 180 node packages/engine/scripts/profile-fitl-preview-drive.mjs --seed 42 --maxTurns 1 --profilesAll --perCard --profileBuckets --label spec150-wasm-initial-full-hash-query-eval-residual-perf` — RED gate with clean active route as recorded above.
- `pnpm run check:ticket-deps` — PASS after creating `tickets/150FITLWASM-025.md` and rewiring dependent tickets.
- `git diff --check` — PASS.
- Post-review markdown cleanup verification: `git diff --check -- tickets/150FITLWASM-024.md tickets/150FITLWASM-025.md` — PASS.

The post-profile ticket/spec/dependency edits transcribed the measured result
and moved successor ownership only; they did not change code, command
semantics, thresholds, or the active-route acceptance boundary, so the decisive
profile remains valid without rerunning.

Schema/artifact fallout: none. The change touches generic runtime hashing only;
no serialized schema, generated JSON, golden, or WASM ABI artifact changed.
