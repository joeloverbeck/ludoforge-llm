...
Implemented the Phase 2 TypeScript runtime slice:

- Added `GameDefRuntime.scheduleIndex` with shared boundary definitions, shared sorted card trigger positions, and run-local `currentDrawPosition` state cloned by `forkGameDefRuntimeForRun`.
- Hooked the live draw effect path so deck draws advance the schedule index for boundaries attached to that draw zone.
- Added policy-runtime support for `schedule.distance.toBoundary.<BoundaryId>.cards`, including ready distance, `notCardScheduled`, `noTriggeringCardRemaining`, and `hiddenDeck` unavailable reasons.
- Added compile-time `SCHEDULE_REF_MISSING_FALLBACK` validation for numeric `value:` expressions that reference schedule-distance refs without `scheduleFallback`.
- Added runtime fallback handling for `noContribution`, constant values, and `dropConsideration`; surfaced fired schedule fallback metadata through policy/microturn candidate traces and the public trace schema.
- Added focused schedule-distance tests covering draw-index correctness, fallback discipline, observer visibility, and byte-pinned card distances.
- Updated the runtime ownership documentation for the new `scheduleIndex` field.

Live-surface corrections from the draft:

- The live draw handler is the generic `applyDraw` path in `packages/engine/src/kernel/effects-token.ts`, not a separate `packages/engine/src/kernel/effects/draw-from-deck.ts` file.
- Public/hidden deck observability already lives on draw zones via `visibility`; this implementation reuses that existing contract instead of adding a new event-deck or data-asset boolean.
- The live policy trace model does not have the generic `inputRefs` row shape described in the draft. This ticket follows the existing fallback-metadata convention by adding `scheduleFallbackFired` to candidate decision metadata and trace schema.
- Runtime `.toPhase` card-distance aliases remain deferred to `tickets/169PHASCHREF-004.md`; this ticket only resolves the canonical `.toBoundary` card unit.
- The focused proof commands use compiled Node test files after `pnpm -F @ludoforge/engine build`; the draft `--test-name-pattern` form is stale for this repo's Node test runner.

Generated/schema fallout:

- `packages/engine/schemas/Trace.schema.json` changed because `PolicyCandidateDecisionTrace` now exposes optional `scheduleFallbackFired` metadata.
- `GameDef.schema.json` and `EvalReport.schema.json` were regenerated by `schema:artifacts` but had no persisted diff.

Implementation-introduced branch coverage:

- `ready` card-distance resolution: covered by `schedule-distance-cards-golden.test.ts` and `schedule-ref-card-draw-index-correctness.test.ts`.
- `hiddenDeck`: covered by `schedule-ref-observer-view.test.ts`.
- `noTriggeringCardRemaining`: covered by `schedule-distance-cards-golden.test.ts` and fallback-discipline tests.
- `notCardScheduled`: covered by the updated existing `phase-identity-refs.test.ts` assertion for card units on an unscheduled boundary.
- `unsupportedScheduleDistance`: retained from the Phase 1 branch coverage in `phase-identity-refs.test.ts`; non-card units and `.toPhase` runtime aliases remain sibling scope.

Source-size ledger:

| Path | Before lines | After lines | Crossed cap? | Active growth | Extraction/defer rationale | Successor |
|---|---:|---:|---|---:|---|---|
| `packages/engine/src/kernel/effects-token.ts` | 1193 | 1194 | No - preexisting oversized | +1 | The draw hook is two direct calls in the existing canonical draw effect path; extracting a new effect module would be a broader effects refactor than this ticket owns. | None |
| `packages/engine/src/cnl/compile-agents.ts` | 4427 | 4490 | No - preexisting oversized | +63 | The fallback diagnostic lives beside the existing consideration validation and ref collection logic; moving only this validation would split one validation pass across files. | None |
| `packages/engine/src/agents/policy-runtime.ts` | 662 | 694 | No | +32 | The card-distance resolver belongs with the existing policy ref providers and stays below the 800-line cap. | None |
| `packages/engine/src/agents/policy-agent.ts` | 886 | 907 | No - preexisting oversized | +21 | Metadata propagation is a narrow addition to established frontier/delegation structures; extracting would not reduce the real hub complexity. | None |
| `packages/engine/src/agents/policy-evaluation-core.ts` | 1943 | 2009 | No - preexisting oversized | +66 | Schedule fallback firing is handled where numeric term contributions are evaluated and candidate metadata is assembled. A separate abstraction can be considered when more fallback families share this code. | None |
| `packages/engine/src/kernel/schemas-core.ts` | 2682 | 2687 | No - preexisting oversized | +5 | Schema addition mirrors the existing trace schema object; no standalone extraction is meaningful. | None |
| `packages/engine/src/kernel/types-core.ts` | 2259 | 2264 | No - preexisting oversized | +5 | Public trace type grows in the canonical kernel type hub to keep schema/type synchronization local. | None |
| `packages/engine/src/agents/policy-eval.ts` | 1498 | 1501 | No - preexisting oversized | +3 | Candidate metadata passthrough is a small addition to an existing trace assembly hub. | None |

Verification:

- Initial red proof after adding tests: `pnpm -F @ludoforge/engine build` failed on the missing schedule index/runtime/fallback surfaces.
- `pnpm -F @ludoforge/engine build` — passed after implementation.
- Focused new schedule tests after build — passed:
  `dist/test/unit/agents/schedule-ref-card-draw-index-correctness.test.js`,
  `dist/test/unit/agents/schedule-ref-fallback-discipline.test.js`,
  `dist/test/unit/agents/schedule-ref-observer-view.test.js`,
  `dist/test/unit/agents/schedule-distance-cards-golden.test.js`.
- Related existing phase tests after build — passed:
  `dist/test/unit/cnl/phase-boundary-compile-validation.test.js`,
  `dist/test/unit/cnl/phase-boundary-determinism.test.js`,
  `dist/test/unit/agents/phase-identity-refs.test.js`,
  `dist/test/determinism/phase-identity-refs-determinism.test.js`.
- `pnpm -F @ludoforge/engine run schema:artifacts` — passed and regenerated trace schema artifacts.
- `pnpm -F @ludoforge/engine run schema:artifacts:check` — passed.
- `pnpm -F @ludoforge/engine test:unit` — passed, 5705 tests.
...
- `pnpm run check:ticket-deps` — passed for 4 active tickets and 2324 archived tickets.
- `git diff --check` — passed.

Late-edit proof validity: after the final code build, only ticket/docs text changed before terminal closeout. The schema check, focused compiled lanes, full unit lane, e2e lane, Turbo engine test filter, typecheck, and lint were all run after implementation code and schema artifacts were stable. The final ticket-text update was followed by dependency and diff-whitespace checks.
