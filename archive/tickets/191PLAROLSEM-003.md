...
- Added generic `root.compound` witness validation in `packages/engine/src/cnl/validate-agent-plan-templates.ts`, with a template-named diagnostic code for unprovable compound metadata. The validator now proves root action/tag reachability, special tag grantability, pipeline accompanying-op compatibility, and `interruptAfterStage`/`timing` consistency against authored action/pipeline surfaces.
- Added `packages/engine/test/architecture/fixtures/agent-plan-compound-witness-fixture.ts` plus unit coverage for valid compound metadata, ungrantable metadata rejection, and invalid interrupt-stage rejection.
- Verified `packages/engine/src/cnl/compile-agent-plan-templates.ts` needed no edit: validation can read the source `GameSpecDoc` action/pipeline surface before compilation copies compound metadata.
- Corrected the FITL profile by removing ungrantable operation-to-operation compound templates `us.airLiftTrain`, `nva.terrorFutureRally`, and `vc.rallyResetTerror`; regenerated the runner FITL bootstrap fixture and updated policy-profile-quality witnesses to cover the remaining valid templates and assert the invalid template IDs are absent.

Verification:

- `pnpm -F @ludoforge/engine build` — passed.
- `node --test dist/test/unit/cnl/agent-plan-template-validate.test.js dist/test/unit/cnl/agent-plan-template-compile.test.js` — passed, 9/9 tests.
- `node --test dist/test/integration/agents/compiled-policy-determinism.test.js` — passed.
- `node --test dist/test/integration/agents/seat-agg-e2e.test.js` — passed.
- `node --test dist/test/integration/fitl-scenario-leader-box-init.test.js` — passed.
- `node --test dist/test/integration/gamedef-cache-equivalence.test.js` — passed.
- `node --test dist/test/policy-profile-quality/us-advise-airlift-force-multiplier.test.js dist/test/policy-profile-quality/vc-protects-bases-from-nva-infiltrate.test.js` — passed.
- `pnpm -F @ludoforge/engine test:all` — passed, 959/959 tests.
- `pnpm -F @ludoforge/runner bootstrap:fixtures` — passed, regenerated 2 bootstrap fixtures.
- `pnpm -F @ludoforge/runner bootstrap:fixtures:check` — passed, fixtures current.
- `pnpm turbo lint` — passed.
- `pnpm turbo typecheck` — passed.

Source-size ledger:

- `packages/engine/src/cnl/validate-agent-plan-templates.ts`: 668 lines, +148 active lines, under the 800-line cap; no split needed.
- `packages/engine/src/cnl/compiler-diagnostic-codes.ts`: 459 lines, +1 active line, under the cap.
- `packages/engine/test/unit/cnl/agent-plan-template-validate.test.ts`: 215 lines, under the cap.
- `packages/engine/test/unit/cnl/agent-plan-template-compile.test.ts`: 210 lines, under the cap.
- `packages/engine/test/architecture/fixtures/agent-plan-compound-witness-fixture.ts`: 152 lines, under the cap.
