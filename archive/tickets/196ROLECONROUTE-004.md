...
`archive/tickets/196ROLECONROUTE-005A.md` owns the bounded post-state role-constraint substrate needed before this series can truthfully claim origin-control preservation as constraint admissibility. `196ROLECONROUTE-005` owns the concrete generic control-preservation constraint and FITL migration on top of that substrate. Neither ticket may invent fake zone ids or game-specific engine logic.
...
- **Origin-control preservation as role-constraint admissibility** — deferred to `archive/tickets/196ROLECONROUTE-005.md`, because the currently landed generic constraints cannot express zone-set membership or post-move control preservation truthfully.
