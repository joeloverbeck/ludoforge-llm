...
**Status**: ✅ COMPLETED 2026-05-29 (re-scoped — the "bound the enumeration" approach below was rejected; resolved by distilling `fitl-spec-143`; see Investigation Outcome + Resolution)

## Investigation Outcome (2026-05-29) — read before re-attempting

Implementing the "bound the enumeration" approach surfaced findings that **invalidate this ticket as written** (full detail in `archive/specs/207-fitl-agent-decision-cost-regression.md` → "⚠ Re-scope" §):
...
4. **Two of the four Spec 207 witnesses were mis-attributed** — `arvn-action-distribution-not-dominated` and `turn-shape-minimum-impact-observed` fail behaviorally on the unmodified baseline (plan-controller domination), unrelated to preview cost → split to **`specs/208-fitl-arvn-baseline-pq-witness-failures.md`**.
...
### Resolution (2026-05-29, FOUNDATIONS reassessment)

The open config-vs-ceiling decision was resolved per `docs/FOUNDATIONS.md` Appendix + `.claude/rules/testing.md`: **distill** `fitl-spec-143-cost-stability.test.ts` to the seed-independent architectural invariant it actually guards — *no agent-retained state that grows with decision count* (`planExecutionState`/`previewWideningState` bounded by game structure; observed ≤1 per agent across ~206 decisions; asserted ≤16) + bounded termination. Phase 1 proved the guarded defect class (retained-state leak) is absent, so the decile wall-time ratio was a false-positive proxy firing on legitimate `deep1024` cost composition (Foundation 10/15). The distilled witness passes un-skipped with **no engine or game-data change**. The three other Spec-207 witnesses moved to `specs/208-fitl-arvn-baseline-pq-witness-failures.md`. Full rationale in `archive/specs/207-fitl-agent-decision-cost-regression.md` → Outcome §.
...
**Deps**: `archive/specs/207-fitl-agent-decision-cost-regression.md`
