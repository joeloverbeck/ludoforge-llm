...
- `proof gaps`: `resolved by tickets/AGENTSUNSET-005.md: policy-profile-quality convergence witnesses are non-blocking quality signals, so the truthful closure was to remove them from the default blocking lane after direct reproduction showed stale expectations rather than a remaining engine-contract regression; later default-lane quiet-progress shifted to unrelated dist/test/integration/spec-140-profile-migration.test.js outside this engine-contract ticket`
