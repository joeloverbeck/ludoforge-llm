...
1. Transfer endpoint normalization is centralized in `model-utils`, but transfer-only contracts and guard names still use generic `ScopeEndpoint*` naming (`ScopeEndpointPayloadObject`, `ScopeEndpointDisplayInput`, `asScopeEndpointPayloadObject`, `invalidEndpointScope`).
2. `trace-projection.test.ts` and `translate-effect-trace.test.ts` each define identical local `catchError` helpers.
3. `model-utils.test.ts` exercises endpoint display/normalization APIs directly, so transfer naming changes will require test import/callsite updates there as well.
4. **Mismatch + correction**: architecture is functionally correct, but naming and test-helper duplication still leave clarity/maintenance debt.
...
Keep `ScopeKind` / `formatScopePrefixDisplay` unchanged because they are shared beyond transfer endpoint rendering.
...
1. `packages/runner/test/model/model-utils.test.ts` - rename imports/calls to transfer-explicit API names; keep behavioral assertions unchanged.
...
- What changed:
  - Renamed transfer-only utility surface in `model-utils` to explicit transfer names (`formatTransferEndpointDisplay`, `TransferEndpointDisplayInput`, `TransferEndpointPayloadObject`, `asTransferEndpointPayloadObject`, `invalidTransferEndpointScope`), with dependent import/callsite updates in trace translation/projection code and tests.
  - Extracted transfer endpoint contracts/helpers into dedicated module `packages/runner/src/model/transfer-endpoint-utils.ts` and removed them from `model-utils` to enforce explicit bounded ownership at file/module level.
  - Added shared test helper `packages/runner/test/model/helpers/catch-error.ts` and removed duplicated inline `catchError` implementations from both trace-related test files.
  - Split mixed model utility tests by module responsibility via `packages/runner/test/model/transfer-endpoint-utils.test.ts`.
- Deviations from original plan:
  - Explicitly updated `packages/runner/test/model/model-utils.test.ts` as part of the rename rollout (this dependency was real in current code and added during reassessment).
  - `packages/runner/src/model/trace-projection.ts` did not require changes because it only depends on `normalizeTransferEndpoint`, whose signature was intentionally kept stable.
  - No runtime behavior changes were introduced; this stayed a naming/structure cleanup.
- Verification results:
  - `pnpm -F @ludoforge/runner test -- trace-projection translate-effect-trace model-utils` passed.
  - `pnpm -F @ludoforge/runner test -- model-utils transfer-endpoint-utils trace-projection translate-effect-trace` passed.
