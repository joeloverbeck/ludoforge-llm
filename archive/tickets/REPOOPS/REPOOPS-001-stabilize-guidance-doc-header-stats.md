# REPOOPS-001: Stabilize Guidance-Doc Header Stats
...
Repository indexing tooling rewrites numeric repository stats in `AGENTS.md` and `CLAUDE.md` (`symbols`, `relationships`, `execution flows`). These values are operationally non-critical and create noisy unrelated diffs that increase merge churn and reduce signal in implementation commits.

## Assumption Reassessment (2026-03-05)

1. The repository index can be stale relative to `HEAD`; after re-running the indexer, only numeric stat lines changed in `AGENTS.md` and `CLAUDE.md`.
2. There is currently no dedicated guard that prevents stat-only header-counter churn from being mixed into feature/bugfix commits.
3. Current script test coverage (`scripts/check-ticket-deps.test.mjs`) does not cover this header-stat churn case.
...
1. Keep a single canonical policy: guidance-doc counter updates are allowed only as dedicated maintenance-only changes.
...
### 1. Add deterministic guard behavior for guidance-doc header counters
...
- `tickets/REPOOPS-001-stabilize-guidance-doc-header-stats.md` (this reassessment)
...
1. Repository workflow has a deterministic policy for guidance-doc header stat churn.
...
2. `node --test <guidance-doc header-stat guard tests>`
3. `pnpm test`
...
- **What changed**
1. Added a dedicated guard enforcing that counter-only churn in `AGENTS.md`/`CLAUDE.md` cannot be mixed with unrelated file changes.
2. Added guard tests with scenario coverage for clean state, isolated counter-only churn, blocked mixed-purpose churn, and non-counter guidance edits.
3. Wired guard into root workflow via `package.json` scripts:
   - a dedicated guidance-doc header-stat guard
   - `test` now runs `check:ticket-deps` plus that guard before `turbo test`.
...
2. The dedicated guidance-doc header-stat guard tests passed.
