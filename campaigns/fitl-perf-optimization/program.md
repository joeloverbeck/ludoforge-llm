# Campaign: fitl-perf-optimization

## Objective

Minimize the total wall-clock time to compile and simulate 3 Fire in the Lake
games (seeds 1000-1002, 4 players each, max 200 turns per game).

**Lower is better.**

FITL is a 4-faction COIN-series wargame with 130 event cards, complex trigger
chains, nested effect loops, and multi-step operations. It is the most complex
game spec in the LudoForge engine and serves as the primary stress test for
engine performance.

## Primary Metric

`combined_duration_ms` — total time for one-time compilation + 3 game
...
**Never guess bottlenecks. Always profile first.** If an optimization
...
Starting hypotheses for the first experiments (derived from FITL's complexity
profile — verify with actual profiling data before acting):

1. **Trigger dispatch is a linear scan**: `dispatchTriggers` iterates ALL
   `def.triggers` for every event. FITL has ~100+ triggers (far more than
   Texas Hold'em's ~10). Indexing by event type would reduce iterations
   dramatically.
2. **Effect chain depth**: FITL operations (train, sweep, assault) involve
   nested `forEach` loops over zones and tokens, each creating intermediate
   state copies via immutable spread. Batching or transient mutation with
   final freeze could reduce allocation pressure.
3. **Condition evaluation overhead**: FITL conditions are complex (aggregate
   counts, zone filters, terrain checks, faction-specific predicates).
   `evalCondition`/`evalValue` are called 200+ times per move and may
   allocate fresh resource objects on every call.
4. **Redundant runtime construction in agent lookahead**: `evaluateState()`
   ...
   every lookahead evaluation.
5. **Compilation cost dominance**: FITL's spec is far larger than Texas
   Hold'em (130 events, extensive macros). With only 5 games per run,
   compilation time may be a larger fraction of `combined_duration_ms`.
6. **Zobrist hash recomputation**: `computeFullHash` may recompute from
   scratch rather than incrementally updating based on the state diff.
7. **legalMoves combinatorial explosion**: FITL operations have many
   parameter combinations (zones × tokens × factions). Early pruning of
   impossible parameter combinations could reduce work significantly.
