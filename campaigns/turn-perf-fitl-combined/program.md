# Campaign: turn-perf-fitl-combined

## Objective

Minimize the combined wall-clock time of the 5 slowest FITL event-card test suites (the actual per-turn bottleneck) by optimizing production engine code. These suites represent the critical path for iterative agent improvement — at ~15 seconds per FITL turn, agent development is infeasible without significant speedup.

Target suites:
1. `fitl-events-plei-mei.test.js` (~30,470ms)
2. `fitl-events-sihanouk.test.js` (~21,560ms)
3. `fitl-events-brinks-hotel.test.js` (~19,680ms)
4. `fitl-events-ia-drang.test.js` (~18,780ms)
5. `fitl-events-fact-finding.test.js` (~18,160ms)

Correctness guards (enforced via full test suite gate, not in metric):
- `policy-agent.perf.test.js`
- `fitl-policy-agent.test.js`
- `fitl-card-flow-determinism.test.js`
...
**Expansion policy**: The mutable scope is intentionally broad. The agent should not be artificially constrained to a pre-identified set of files. If optimization reveals hot paths in files not initially identified in root causes, expand attention freely and follow the evidence wherever it leads.
