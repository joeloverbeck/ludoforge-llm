...
Convergence claims tied to a specific policy-profile variant are not engine invariants. They are quality signals for the profile maintainer, and they live in `packages/engine/test/policy-profile-quality/`, not in `determinism/`. They still emit `POLICY_PROFILE_QUALITY_REGRESSION` signals and a PR report, but — amended 2026-05-29 — the full `policy-profile-quality` lane now runs as a **blocking** CI lane (its own job in `engine-determinism.yml`), not a non-blocking summary. The earlier non-blocking treatment let witnesses silently rot (a curated subset ran, `continue-on-error` swallowed the rest), which is the opposite of "testing as proof." A failing profile-quality witness must therefore be resolved, not ignored: fix the regression; or, when the seed-pinned trajectory has legitimately drifted, **distill** it to a seed-independent architectural invariant (preferred) or re-bless it (per `.claude/rules/testing.md`); or, when it has caught a real but not-yet-fixed regression, **quarantine** it with an explicit `skip` referencing a tracking spec (never by relaxing the asserted bound). This keeps the architectural separation below intact — these tests still assert profile-quality, not engine determinism — while ensuring the lane cannot quietly accumulate unverified or failing witnesses.
