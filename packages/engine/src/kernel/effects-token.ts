...
const resolveTokenOccurrence = (
  ...
  const tokenState = getTokenStateIndexEntry(ctx.state, tokenId, ctx.resources.tokenStateIndexCache);
...
export const applyMoveToken = (
  ...
  emitTrace(env.collector, {
    ...
    from: fromZoneId,
    to: toZoneId,
    provenance: resolveTraceProvenance(traceCtx),
  });
  ...
  const table = cursor.tracker && env.skipRunningHashUpdates !== true ? env.cachedRuntime?.zobristTable : undefined;
  if (table !== undefined) {
...
export const applyDestroyToken = (
  ...
  if (cursor.tracker && env.skipRunningHashUpdates !== true) {
