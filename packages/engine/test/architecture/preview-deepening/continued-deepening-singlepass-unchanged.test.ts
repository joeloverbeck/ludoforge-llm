...
describe('continued deepening singlePass dispatch seam', () => {
  ...
  it('keeps continuedDeepening on the broad-only path until the deep driver lands', () => {
    assert.equal(
      JSON.stringify(capturePreview('continuedDeepening')),
      JSON.stringify(capturePreview('singlePass')),
    );
  });
  ...
  it('keeps the strategy dispatch insertion site explicit for Ticket 004', () => {
    ...
    assert.doesNotMatch(source, /policy-preview-inner-deepening/u);
  });
});
