...
actor: 'active',
  ...
  it('uses selected scenario initialTrackValues as canonical track initializers', () => {
    const markdown = [
      '```yaml',
      'metadata:',
  ...
  it('derives gameDef factions from the piece catalog selected by scenario', () => {
    ...
    assertNoDiagnostics(compiled);
    assert.notEqual(compiled.gameDef, null);
    assert.deepEqual(compiled.gameDef?.factions, [{ id: 'nva', color: '#2a9d8f' }]);
    ...
    assert.equal(
      compiled.diagnostics.some(
        (diagnostic) =>
          ...
          diagnostic.path === 'doc.dataAssets.1.payload.factions',
      ),
      true,
    );
  });
