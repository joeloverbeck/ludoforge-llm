...
describe('FITL 1964 remaining event-card production spec', () => {
  ...
  it('encodes card 15 (Medevac) as canonical momentum lasting effects on both sides', () => {
    ...
    assert.deepEqual(stripEffectFootprints(unshadedMomentum?.setupEffects), tagEffectAsts([{ setVar: { scope: 'global', var: 'mom_medevacUnshaded', value: true } }]));
    assert.deepEqual(stripEffectFootprints(unshadedMomentum?.teardownEffects), tagEffectAsts([{ setVar: { scope: 'global', var: 'mom_medevacUnshaded', value: false } }]));
    ...
    assert.deepEqual(stripEffectFootprints(shadedMomentum?.setupEffects), tagEffectAsts([{ setVar: { scope: 'global', var: 'mom_medevacShaded', value: true } }]));
    assert.deepEqual(stripEffectFootprints(shadedMomentum?.teardownEffects), tagEffectAsts([{ setVar: { scope: 'global', var: 'mom_medevacShaded', value: false } }]));
  });
  ...
  it('encodes card 31 (AAA) as capability marker toggles for cap_aaa', () => {
    ...
    assert.deepEqual(stripEffectFootprints(card?.unshaded?.effects), tagEffectAsts([{ setGlobalMarker: { marker: 'cap_aaa', state: 'unshaded' } }]));
    assert.deepEqual(stripEffectFootprints(card?.shaded?.effects), tagEffectAsts([{ setGlobalMarker: { marker: 'cap_aaa', state: 'shaded' } }]));
  });
