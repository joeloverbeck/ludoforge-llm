...
describe('FITL 1968 US-first event-card production spec', () => {
  ...
  it('encodes card 16 (Blowtorch Komer) as unshaded round momentum toggle', () => {
    ...
    assert.deepEqual(stripEffectFootprints(momentum?.setupEffects), tagEffectAsts([{ setVar: { scope: 'global', var: 'mom_blowtorchKomer', value: true } }]));
    assert.deepEqual(stripEffectFootprints(momentum?.teardownEffects), tagEffectAsts([{ setVar: { scope: 'global', var: 'mom_blowtorchKomer', value: false } }]));
  });
  ...
  it('encodes card 2 (Kissinger) metadata, text, and shaded aid penalty', () => {
    ...
    assert.deepEqual(stripEffectFootprints(card?.shaded?.effects?.at(-1)), tagEffectAsts([{ addVar: { scope: 'global', var: 'aid', delta: -6 } }])[0]);
  });
