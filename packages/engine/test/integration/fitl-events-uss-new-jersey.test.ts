...
describe('FITL card-30 USS New Jersey', () => {
  ...
  it('unshaded allows any 1..3 coastal spaces without in-space COIN and applies max-2-per-space removals with no Trail degrade', () => {
    const def = compileDef();
    ...
    assert.throws(
      () =>
        applyMoveWithResolvedDecisionIds(def, setup, {
          ...eventMove!,
          params: { ...eventMove!.params, $coastalSpaces: [NON_COASTAL_SPACE] },
        }),
    ...
    assert.throws(
      () =>
        applyMoveWithResolvedDecisionIds(def, setup, {
          ...eventMove!,
          params: { ...eventMove!.params, $coastalSpaces: [] },
        }),
      /Illegal move|outside options domain|cardinality mismatch|ACTION_NOT_LEGAL_IN_CURRENT_STATE/,
    ...
    assert.throws(
      () =>
        applyMoveWithResolvedDecisionIds(def, setup, {
          ...
          params: {
            ...eventMove!.params,
            $coastalSpaces: [COASTAL_CITY, SECOND_COASTAL_SPACE, 'saigon:none', 'cam-ranh:none'],
          },
        }),
      /Illegal move|outside options domain|cardinality mismatch|ACTION_NOT_LEGAL_IN_CURRENT_STATE/,
    ...
    const after = applyMoveWithResolvedDecisionIds(def, setup, {
      ...eventMove!,
      params: { ...eventMove!.params, $coastalSpaces: [COASTAL_CITY] },
    }).state;
