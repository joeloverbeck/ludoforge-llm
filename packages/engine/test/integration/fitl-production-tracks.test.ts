...
describe('FITL production numeric tracks', () => {
  it('encodes all FITL numeric tracks with expected scope and bounds', () => {
    const tracks = readMapTracks();
    assert.equal(tracks.length, 9);

    const byId = new Map(tracks.map((track) => [track.id, track]));
    assert.equal(byId.size, 9, 'Track IDs must be unique');
    ...
    assert.equal(globalTracks.length, 6);
    ...
    assert.deepEqual(
      ...
      ['aid', 'leaderBoxCardCount', 'patronage', 'terrorSabotageMarkersPlaced', 'totalEcon', 'trail'],
    );
    ...
    assert.equal(byId.get('leaderBoxCardCount')?.min, 0);
    assert.equal(byId.get('leaderBoxCardCount')?.max, 8);
