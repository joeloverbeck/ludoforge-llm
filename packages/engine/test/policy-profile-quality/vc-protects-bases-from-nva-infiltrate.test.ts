...
describe('Spec 188 VC Base protection and NVA-rival witness', () => {
  it('binds Rally/Subvert skeletons to Base protection and NVA ally-rival wiring', () => {
    ...
    const terrorTemplate = fixture.def.agents?.library.planTemplates?.['vc.rallyResetTerror'];
    const rallyResult = proposeVcPlan(fixture, ['rally']);
    const rallySubvert = requireAlternative(rallyResult, 'vc.rallySubvert');
    const rallyResetTerror = requireAlternative(rallyResult, 'vc.rallyResetTerror');
    const passed = relationship?.seat === 'nva'
      ...
      && terrorTemplate?.roles.terrorSpace?.selectorId === 'vc.terrorAgitationSpace'
      && rallySubvert.roleBindings.rallySpace?.components.baseProtection !== undefined
      && rallyResetTerror.roleBindings.rallySpace?.components.undergroundReset === 1;
    ...
    assert.equal((profile?.plan.planTemplates ?? []).includes('vc.rallyResetTerror'), true);
    assert.equal(rallyTemplate?.roles.rallySpace?.selectorId, 'vc.rallyBaseOrUndergroundSpace');
    assert.equal(terrorTemplate?.roles.terrorSpace?.selectorId, 'vc.terrorAgitationSpace');
    assert.notEqual(rallySubvert.roleBindings.rallySpace?.components.baseProtection, undefined);
    assert.equal(rallyResetTerror.roleBindings.rallySpace?.components.undergroundReset, 1);
  });
});
