  ...
  it('lowers every authored preview.option ref to the compiled preview-option ref family', () => {
    const result = compileGameSpecToGameDef({
      ...
      agents: {
        library: {
          considerations: {
            margin: { scopes: ['microturn'], weight: 1, value: { coalesce: [refExpr('preview.option.victory.currentMargin.self'), 0] } },
            rank: { scopes: ['microturn'], weight: 1, value: { coalesce: [refExpr('preview.option.victory.currentRank.self'), 0] } },
            delta: { scopes: ['microturn'], weight: 1, value: { coalesce: [refExpr('preview.option.delta.victory.currentMargin.self'), 0] } },
            globalVar: { scopes: ['microturn'], weight: 1, value: { coalesce: [refExpr('preview.option.var.global.score'), 0] } },
            playerVar: { scopes: ['microturn'], weight: 1, value: { coalesce: [refExpr('preview.option.var.player.self.tempo'), 0] } },
            metric: { scopes: ['microturn'], weight: 1, value: { coalesce: [refExpr('preview.option.metric.pressure'), 0] } },
            outcome: {
              scopes: ['microturn'],
              weight: 1,
              value: { boolToNumber: { eq: [refExpr('preview.option.outcome'), 'ready'] } },
            },
            driveDepth: { scopes: ['microturn'], weight: 1, value: { coalesce: [refExpr('preview.option.driveDepth'), 0] } },
          },
