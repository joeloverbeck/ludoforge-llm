...
describe('crossValidateSpec', () => {
  ...
  it('eventDeck side targets with target-local effects produce no executability diagnostics', () => {
    const sections = compileRichSections();
    const deck = requireValue(sections.eventDecks?.[0]);
    const card = requireValue(deck.cards[0]);
    const diagnostics = crossValidate({
  ...
  it('eventDeck branch targets with target-local effects produce no executability diagnostics', () => {
    const sections = compileRichSections();
    const deck = requireValue(sections.eventDecks?.[0]);
    const card = requireValue(deck.cards[0]);
    const diagnostics = crossValidate({
