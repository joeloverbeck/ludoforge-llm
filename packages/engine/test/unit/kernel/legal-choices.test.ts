...
describe('legalChoicesDiscover()', () => {
  ...
  it('does not satisfy an effect choice from a legacy bind-name param alias', () => {
    ...
    const result = legalChoicesDiscover(def, state, makeMove('legacyAliasChoiceAction', { 'decision:$target': 'a' }));
    assert.equal(result.kind, 'pending');
    if (result.kind !== 'pending') {
      throw new Error('expected pending request');
    }
    assert.equal(result.decisionKey, '$target');
...
phase: [asPhaseId('main')],
    ...
    assert.equal(result1.decisionKey, '$color');
...
phase: [asPhaseId('main')],
    ...
    const r2 = legalChoicesDiscover(def, state, makeMove('multiChoice', { '$first': 'x' }));
    ...
    const r3 = legalChoicesDiscover(def, state, makeMove('multiChoice', { '$first': 'x', '$second': 'b' }));
...
phase: [asPhaseId('main')],
    ...
    assert.equal(result.decisionKey, '$innerChoice');
...
phase: [asPhaseId('main')],
    ...
    const r2 = legalChoicesDiscover(def, state, makeMove('exactPick', { '$exactTargets': ['alpha', 'gamma'] }));
...
phase: [asPhaseId('main')],
    ...
    it('throws contextual runtime error when profile legality evaluation fails', () => {
      ...
      const profile: ActionPipelineDef = {
        ...
        legality: { op: '==', left: { _t: 2, ref: 'gvar', var: 'missingVar' }, right: 1 },
        costValidation: null,
        costEffects: [],
        targeting: {},
        stages: [],
    ...
    it('treats missing-binding costValidation as deferred during discovery', () => {
      ...
      const profile: ActionPipelineDef = {
        ...
        legality: null,
        costValidation: { op: '==', left: { _t: 2, ref: 'binding', name: '$target' }, right: 'a' },
        costEffects: [],
        targeting: {},
        stages: [
          {
              ...
              effects: [
                eff({
                  if: {
                    when: {
                      op: '==',
                      left: { _t: 2, ref: 'tokenProp', token: '$piece', prop: 'type' },
                      right: 'base',
                    },
                    then: [
                      {
    ...
    it('allows later stage predicates to consume transient bindings exported by earlier stage effects', () => {
      ...
      const profile: ActionPipelineDef = {
        ...
        stages: [
          ...
          {
            legality: { op: '==', left: { _t: 2, ref: 'binding', name: '$mode' }, right: 'allowed' },
            effects: [],
          },
        ],
        atomicity: 'atomic',
...
phase: [asPhaseId('main')],
      ...
      assert.equal(request.decisionKey, '$targetProvince');
      assert.deepEqual(request.options.map((option) => option.value), ['board:cambodia']);
    });

    it('defers unresolved bindings on no-zone free-operation zone-filter path during discovery', () => {
      const action: ActionDef = {
        id: asActionId('operation'),
actor: 'active',
executor: 'actor',
phase: [asPhaseId('main')],
    ...
    it('defers unresolved bindings on per-zone free-operation zone-filter path during discovery', () => {
      const action: ActionDef = {
        id: asActionId('operation'),
actor: 'active',
executor: 'actor',
phase: [asPhaseId('main')],
      ...
      const request = legalChoicesDiscover(def, state, {
        actionId: asActionId('operation'),
    ...
    it('resolves multi-unresolved zone aliases deterministically during free-operation zone-filter probing', () => {
      const action: ActionDef = {
        id: asActionId('operation'),
actor: 'active',
executor: 'actor',
            ...
            pendingFreeOperationGrants: [
              {
                ...
                zoneFilter: {
                  op: '==',
                  left: { _t: 2, ref: 'gvar', var: 'missingVar' },
                  right: 1,
                },
                remainingUses: 1,
              },
