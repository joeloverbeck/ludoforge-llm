...
import { buildSeatResolutionIndex, normalizeSeatKey, resolvePlayerIndexForSeatValue } from '../../../src/kernel/index.js';
...
describe('seat-resolution helpers', () => {
  ...
  it('resolves player indices across seat ids, mapped card seat keys, normalized values, and numeric fallback', () => {
    const def = {
      seats: [{ id: 'us' }, { id: 'nva' }],
      turnOrder: {
        type: 'cardDriven',
    ...
    assert.equal(resolvePlayerIndexForSeatValue('1', 2, index), 1);
