...
describe('effect grant sequence-context path traversal policy', () => {
  it('forbids recovering control-flow semantics from pathSuffix string comparisons', () => {
    const source = readKernelSource('src/kernel/effect-grant-sequence-context-paths.ts');

    assert.equal(
      /\.pathSuffix\s*===\s*['"`]\./u.test(source),
      false,
      'effect-grant-sequence-context-paths.ts must not branch on serialized pathSuffix values',
    );
    assert.equal(
      /nestedScope\.pathSuffix\s*===/u.test(source),
      false,
      'effect-grant-sequence-context-paths.ts must not compare nestedScope.pathSuffix directly',
    );
  });
  ...
  it('requires traversal semantics to come from helper-owned descriptor metadata', () => {
    ...
    assert.equal(
      ...
      true,
      'effect-grant-sequence-context-paths.ts must branch on helper-owned traversal metadata',
    );
  });
});
