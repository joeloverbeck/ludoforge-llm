...
describe('createLayerHierarchy', () => {
  it('returns all named containers with expected event modes', () => {
    ...
    expect(layers).toMatchObject({
      ...
      provinceZoneLayer: expect.any(Container),
      connectionRouteLayer: expect.any(Container),
      cityZoneLayer: expect.any(Container),
    ...
    expect(layers.provinceZoneLayer.eventMode).toBe('passive');
    expect(layers.cityZoneLayer.eventMode).toBe('passive');
  ...
  it('places board-space layers inside board group in order', () => {
    ...
    expect(layers.provinceZoneLayer.parent).toBe(layers.boardGroup);
    expect(layers.connectionRouteLayer.parent).toBe(layers.boardGroup);
    expect(layers.cityZoneLayer.parent).toBe(layers.boardGroup);
    ...
    expect(layers.boardGroup.children[2]).toBe(layers.provinceZoneLayer);
    expect(layers.boardGroup.children[3]).toBe(layers.connectionRouteLayer);
    expect(layers.boardGroup.children[4]).toBe(layers.cityZoneLayer);
    expect(layers.boardGroup.children[5]).toBe(layers.adjacencyLayer);
    expect(layers.boardGroup.children[6]).toBe(layers.tableOverlayLayer);
  });
