...
`archive/tickets/179ACTSELPRE-008.md` completed the Phase 2 witness contract reset. No valid replacement witness exists in the current FITL event/free-operation path, so Spec 179 remains blocked/deferred. `archive/tickets/179ACTSELPRE-009.md` selected Spec 180's bounded standing-projection route as the ordinary-operation successor; implementation starts with the focused failing witness in `tickets/180STDVECOBSROL-001.md`. Tickets 005, 006, and 007 remain blocked and not archive-ready until the Spec 180 successor proves the replacement surface.
