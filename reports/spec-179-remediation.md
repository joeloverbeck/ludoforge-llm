...
**Disposition summary**:
- **Spec 179** (`specs/179-action-selection-preview-outcome-grant-opt-in.md`) **KEPT UNCHANGED**. The trigger report's trace data shows opponent refs are *requested AND `ready` at 75.3%* with uniform values across candidates — the cause is the drive boundary at `policy-preview.ts:986-993` (mechanical), not status fidelity. Spec 179's `outcomeGrantContinuation` opt-in is the correct fix for the witness gap and the lowest-cost path that preserves Foundation #10.
- **Spec 180** (`specs/180-standing-vector-observability-and-outer-preview-signal-integrity.md`) **CREATED** as a scoped follow-up, dependent on Spec 179 landing first. Captures the durable improvements from this report that are not addressed by 179: (a) status-aware `seatAgg` for the outer-preview path (extends Foundation #20 from inner-preview where Spec 162 wired it); (b) per-candidate × per-seat trace matrix in `previewUsage` (observability); (c) named role primitives `currentLeader`/`nearestThreat`/`closestAhead`/`closestBehind` resolved from `def.terminal.ranking` (ergonomics, game-agnostic).
