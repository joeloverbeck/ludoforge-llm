...
1. Load the bundle (cheap; mostly file reads and the existing fingerprint computation).
2. Probe the persistent cache for `(gameKey, sourceFingerprint, cacheFormatVersion)`.
3. On hit: deserialize the cached entry, run `assertValidatedGameDef` on the GameDef, populate the in-process cache, return.
