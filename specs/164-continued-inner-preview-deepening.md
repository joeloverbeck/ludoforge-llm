...
**Status**: PROPOSED
**Date**: 2026-05-09
**Predecessors**: Spec 162 (preview signal integrity / Foundation #20), Spec 161 (chooseNStep inner preview integration), Spec 160 (per-option preview at inner microturns), Spec 159 (preview policy-guided completion), Spec 156 (preview observability and utility metrics).
...
4. Default behavior is **unchanged** for profiles that do not opt in. `INNER_PREVIEW_HARD_CAP = 256` remains the default cap; the existing single-pass driver remains the default strategy. No fixture migration is forced by this spec.
...
```ts
// packages/engine/src/cnl/compile-agents.ts:1022-1024
...
bounded by `INNER_PREVIEW_HARD_CAP = 256` (`compile-agents.ts:85`).

`CompiledAgentPreviewInnerConfig` (`packages/engine/src/agents/types-core.ts:870-876`) has no `strategy` field today:
...
The inner driver (`policy-preview-inner.ts:318-422`) operates on a `MutableState` that the broad pass already fully constructs and returns through `DriveResult` (`policy-preview-inner.ts:198-205`). The `state` field on `DriveResult` is exactly the post-broad checkpoint a deep pass would resume from — there is no separate replay-certificate machinery to introduce. State handoff between broad and deep is a matter of carrying the `DriveResult.state` forward and re-driving with a higher `depthCap`, not a new replay protocol.
...
- **No raise of the default hard cap.** Profiles that do not declare `strategy` still validate against `INNER_PREVIEW_HARD_CAP = 256`. The new cap classes (`standard256` and `deep1024`) are opt-in tiers, not a global default change. This preserves Foundation #14 (no compatibility shim — old profiles continue to work as-is).
...
Single-pass formula stays as-is (`compile-agents.ts:1022-1024`). Continued-deepening adds a per-phase formula:
...
2. When `strategy === 'continuedDeepening'`: `totalCost ≤ capClass` AND `Db ≤ Dd` AND `Db < depthCap` is rejected (legacy `depthCap` field MUST equal `broad.depthCap` to keep the field's meaning consistent — the compiler enforces this equality and rejects mismatch with `CNL_COMPILER_AGENT_PREVIEW_DEPTHCAP_MISMATCH`).
...
This handoff requires no new replay machinery, no new state-snapshot infrastructure, and no kernel changes. The driver already accepts an arbitrary `MutableState` as input; passing the broad-pass output as input is a one-line wiring change in the deep driver.
...
| Texas Hold'em mid-budget | 4 | 2 | 4 | 8 | 28 | 64 | 92 | `standard256` ✓ |
| FITL conservative deepening | 6 | 1 | 3 | 8 | 78 | 240 | 318 | `deep1024` ✓ |
...
- `packages/engine/src/agents/policy-preview-inner-choosenstep.ts:262, 477` — broad-pass driver (the existing `runChooseNStepInnerPreview` invocation) — no behavioral change. Its output `DriveResult` is the broad checkpoint.
...
- `packages/engine/src/agents/policy-agent.ts` — `chooseFrontierDecision` (line 266) dispatches to either the single-pass driver (current behavior) or the deepening driver based on `previewInner.strategy`. The dispatch is a thin if/else; no shape change to the surrounding code.
...
- `packages/engine/src/agents/types-core.ts` — extend `CompiledAgentPreviewInnerConfig`, add `ContinuedDeepeningConfig`, add `DeepTrigger`, add `PolicyPreviewPhaseCoverage`, extend `PolicyPreviewCoverage`.

Foundation #20's contract is preserved by the deep driver registering `unavailable` statuses for refs that remain unresolved after the deep pass — through the same `unknownPreviewRefs` tracking machinery (`policy-evaluation-core.ts:1459-1528`) that Spec 162 already uses.
...
| 1 | Compiler — strategy/capClass lowering + per-phase cost validation + diagnostics | Round-trip test: `singlePass` profiles compile unchanged; `continuedDeepening` profiles validate against the per-phase cost formula; over-budget profiles rejected; `depthCap`/`broad.depthCap` mismatch rejected | M |
...
- `packages/engine/src/cnl/compile-agents.ts:85` — `INNER_PREVIEW_HARD_CAP = 256` (UNCHANGED; serves as `standard256` budget)
- `packages/engine/src/cnl/compile-agents.ts:1022-1024` — single-pass cost formula (UNCHANGED for `singlePass`; new per-phase formula added alongside)
- `packages/engine/src/cnl/compile-agents.ts:976-1045` — `lowerPreviewInner` (extended to lower `strategy`, `capClass`, `continuedDeepening` block)
- `packages/engine/src/agents/types-core.ts:870-876` — `CompiledAgentPreviewInnerConfig` (shape extension)
...
- `packages/engine/src/agents/policy-preview-inner-choosenstep.ts:262, 477` — broad-pass driver entry points (UNCHANGED)
- `packages/engine/src/agents/policy-agent.ts:266` — `chooseFrontierDecision` (strategy dispatch added)
- `packages/engine/src/agents/policy-evaluation-core.ts:1459-1528` — `unknownPreviewRefs` tracking (UNCHANGED; deep driver feeds the same machinery)
- `docs/FOUNDATIONS.md:63-79` — Foundation #10 (amended in Phase 0)
- `docs/agent-dsl-cookbook.md:251-413` — Inner Preview section (extended in Phase 4)
...
- **Reusable draft states or replay certificates**: Adopted as draft-state continuation (the `DriveResult.state` checkpoint), NOT as replay certificates. Replay certificates were a design alternative that would have introduced new infrastructure; the existing checkpoint suffices because `driveOption` already accepts an arbitrary `MutableState` as input.
