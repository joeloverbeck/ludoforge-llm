...
2. If `ref.surface === 'previewOptionState'`:
   - Require a candidate-bound `DriveResult` in scope. Action-selection candidates and chooseNStep ADD/CONFIRM frontiers without per-option drive context return `unavailable` with reason `unknownGated`. (Mirrors the existing behavior for `preview.option.*` refs at non-preview frontiers per Spec 162.)
...
- `packages/engine/src/agents/policy-evaluation-core.ts` — extend `resolveLookupRef` (Spec 163, around `:1510-1542` mirroring `resolveSurfaceRef`) with the routing rule from §4.3. When `ref.surface === 'previewOptionState'`:
  - If no per-candidate `DriveResult` is in scope, return `unavailable` with reason `unknownGated` and register in `unknownPreviewRefs[]` (NOT `unknownLookupRefs[]` — the unavailability is preview-derived).
...
| 3 | Runtime routing in `resolveLookupRef` | Architectural-invariant test: projected lookup at a chooseOne frontier with a ready drive resolves against `DriveResult.state`; same lookup at a depth-capped drive returns `unavailable(depthCap)` and registers in `unknownPreviewRefs`; same lookup at action-selection without a per-option drive returns `unavailable(unknownGated)` | M |
...
6. **`projected-lookup-gated-at-action-selection.test.ts`** — Projected lookup referenced at an action-selection frontier (no per-option `DriveResult` in scope under the current preview pipeline). Assert: every candidate returns `unavailable(unknownGated)`; consideration's `previewFallback.onUnavailable: noContribution` produces no contribution; `unknownPreviewRefs` records the ref. Mirrors Spec 162's gating of `preview.option.*` at non-preview frontiers.
