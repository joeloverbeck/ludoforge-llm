...
- [`tickets/173DEEPPRVCOST-007.md`](../tickets/173DEEPPRVCOST-007.md) — Phase 1 — Train decision-stack digest residual closure (owns the remaining post-006 train decision-stack encode/digest bucket, or the Phase-N classification if no safe TS-side closure remains)

Phase 1 tickets land sequentially per §4.4 (the post-merge witness output of ticket K informs ticket K+1's scope). The post-005 witness materially improved the selected coup residual but left the §1 slowest-seed soft target red and still reports train add/confirm above the Phase 1 spread criterion. Ticket 006 classified a safe choose-N preview no-entry-hash candidate as non-closeout evidence and handed the remaining concrete digest/encoding owner to ticket 007 before any later govern/event selection or Phase-N escalation decision. Total Phase 1 ticket count depends on what each witness reveals; expect 2–4 more slices per the spec 172 precedent.
