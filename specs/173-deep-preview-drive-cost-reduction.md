...
- [`tickets/173DEEPPRVCOST-005.md`](../tickets/173DEEPPRVCOST-005.md) — Phase 1 — Secondary continuedDeepening residual closure for `coupArvnRedeployPolice:chooseOne` (covers the post-004 final residual after train gates closed; selected from the next non-overlapping continuedDeepening axis in the final witness)

Phase 1 tickets land sequentially per §4.4 (the post-merge witness output of ticket K informs ticket K+1's scope). The post-004 final witness closed the train elapsed gates but left the §1 slowest-seed soft target red, so ticket 005 owns the next non-overlapping secondary continuedDeepening residual selected from that witness. Likely later candidates if 005 does not fully close the §1 soft target: remaining secondary slow-tier classes (`govern:chooseN*`), event-class bytecode-cache miss rate, or any new top axis that emerges after the coup residual recedes. Total Phase 1 ticket count depends on what each witness reveals; expect 2–4 more slices per the spec 172 precedent.
