...
- [`tickets/173DEEPPRVCOST-006.md`](../tickets/173DEEPPRVCOST-006.md) — Phase 1 — Train continuedDeepening residual spread closure (covers the post-005 witness where train add/confirm remain above the Phase 1 fast-vs-slow spread criterion even though their ticket-004 elapsed gates remain closed)

Phase 1 tickets land sequentially per §4.4 (the post-merge witness output of ticket K informs ticket K+1's scope). The post-005 witness materially improved the selected coup residual but left the §1 slowest-seed soft target red and still reports train add/confirm above the Phase 1 spread criterion. Ticket 006 owns that concrete residual classification before any later govern/event selection or Phase-N escalation decision. Total Phase 1 ticket count depends on what each witness reveals; expect 2–4 more slices per the spec 172 precedent.
