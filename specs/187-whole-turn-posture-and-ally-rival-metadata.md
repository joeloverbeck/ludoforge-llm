...
- **No new preview depth or new cap classes.** Posture consumes the preview surface Spec 185 already produces. No raising of `standard256`/`deep1024`/`postGrant16`. If preview is non-`ready`, posture falls back; it never deepens to force a `ready`.
...
A library bucket `postureEvaluators`, each referenced by a plan template's `postureHook`. A posture evaluator declares:

- `must`: hard posture constraints (e.g. resource-floor-after-turn satisfied) that veto/demote a plan;
- `prefer`: weighted posture preferences, each a leaf-scored value over projected state (`preview.plan.delta.*`) with an explicit `when` and an explicit **fallback contribution** for non-`ready` preview (Foundation #20);
...
A library bucket `relationships` mapping generic relationship roles to seats via authored conditions:

- role kinds: `nominalAlly`, `sharedEnemy`, `rivalAlly`, `leader`, `nearWin`, `kingmakerRisk`, `cooperativeUntilThreshold`;
- each binds to a seat (or standing role) via an authored condition (e.g. `standing.<seat>.nearWin`), never a hardcoded faction id;
- exposes refs (`relationship.<role>.seat`, `relationship.<role>.gainValue`) consumable by posture `prefer` terms and selector vetoes.

**Conditional ally weighting** is expressed as a posture `prefer` term whose `when` gates on a relationship ref, e.g. "value `nominalAlly` margin gain at weight W *unless* `relationship.nominalAlly.nearWin`, in which case treat ally gain as enemy gain." This realizes the report's `rival_ally_gain_if_ally_near_win` term generically.

### 4.3 Trace contract extension

The plan trace `posture` block records: `{ status, mustViolations[], preferContributions[], allyWeightContext }`, with each contribution tagged `ready` or its fallback reason. Replay-identical.
...
**Phase 2 — Relationship metadata.** Acceptance: (a) relationship roles compile and bind seats via authored conditions with no faction ids in engine code; (b) a conditional ally-weight term flips an ally's gain from positive to negative when `relationship.nominalAlly.nearWin` holds, in a constructed scenario; (c) the flip is replay-stable and traced.

## 8. Test plan

- Compiler: posture/relationship golden + determinism + authoring-error corpus (posture term missing fallback; relationship role binding unknown seat).
...
#1 (generic relationship/posture metadata, no faction ids in engine) · #4 (observer-safe projection; analysis-only omniscience labeled) · #10 (posture bounded by existing plan caps; no new depth) · #12 (static validation of posture/relationship refs) · #15 (denial + ally-rival scoring as first-class structure, not a weight hack) · #20 (posture preview status honesty; declared fallbacks visible in trace).
...
To be decomposed under `PLANPOSTURE` after Spec 186 lands. Phase 1 (posture) before Phase 2 (relationships), since conditional ally weights are authored as posture terms.
