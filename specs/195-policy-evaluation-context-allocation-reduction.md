...
2. **Eliminate the per-fall-through full constructor cost** for the same-`cacheBinding` case (the only case observed in current code; future paths with binding-mismatch fall through to per-call construction). Heavy substructure (encoded state, layout, runtime providers, the 13 collection caches, `cacheBinding`) is inherited by reference.
...
- [`tickets/195POLEVACON-001.md`](../tickets/195POLEVACON-001.md) — Inner-selector substructure-sharing wrapper at `policy-evaluation-core.ts:2040` (covers §8 P1)
