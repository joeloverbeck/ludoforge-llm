...
- **`vc.subvertHighValueTarget` zone-prop vocabulary** — **P0a (pre-resolve before P1).** Identify the ARVN-Patronage-priority proxy. Candidate proxies: `population` + `supportOpposition` marker (passiveSupport/activeSupport) + sign filter on `feature.projectedArvnMarginDelta`. Resolution recorded in P0a's audit table.
- **`feature.vcUndergroundGuerrillaCount` resolution** — **P0a.** Either (a) author via `globalTokenAgg` with a token-active-state filter (verify the operator supports the filter), (b) re-express via per-zone `lookup` of the Underground marker, or (c) use the coarser `feature.vcGuerrillaCount@95` as the posture's `prefer` value.
...
- [`tickets/204FITLVCCOM-001.md`](../tickets/204FITLVCCOM-001.md) — P0a Capability / vocabulary re-expression audit (covers §6 P0a)
