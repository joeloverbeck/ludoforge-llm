...
Spec 133 Phase 2 requires every `.test.ts`/`.test.mts` file under `packages/engine/test/**` to carry a `@test-class` marker. This ticket covers the `unit/` lane (~95+ files). Unit tests typically assert properties of isolated functions, deterministic helpers, or compiler output — they map naturally to architectural-invariant per §5's "new-test default." This ticket applies the default mechanically and surfaces per-file exceptions requiring convergence-witness classification.

## Assumption Reassessment (2026-04-18)

1. `packages/engine/test/unit/` contains ~95+ `.test.ts`/`.test.mts` files across subdirectories (`kernel/`, `cnl/`, `agents/`, `sim/`, `contracts/`, `lint/`, `trace/`, `infrastructure/`, etc.). Count verified by the Spec 133 reassessment Explore agent.
2. `packages/engine/test/unit/infrastructure/` will contain tickets 001's reporter unit test and 006's meta-test. Those tickets land their own markers on the files they create; this ticket does not pre-create or touch their files.
3. Most unit tests assert purely functional properties or deterministic helper outputs — default classification is `architectural-invariant`. Exceptions are rare but real (e.g., a unit test that pins a specific PRNG sequence for a specific seed as a convergence witness of a past RNG-subsystem fix).
4. Spec 133 `.claude/rules/testing.md` guidance (ticket 002) provides the authoritative classification rules referenced during per-file inspection.
...
2. **Foundation 14 exception applies**: The diff spans 95+ files, but the change per file is uniform: a single-line header comment. The diff is reviewable because the pattern repeats identically; exception classifications (any convergence-witness marks) are explicitly called out in the PR description. Note this mechanical uniformity in review.
