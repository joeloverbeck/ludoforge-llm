...
**Deps**: `tickets/138ENUTIMTEM-004.md`

## Problem

Per Spec 138 Investigation I4 and Goal G5, the subset-extraction pass runs per free-operation template per enumeration. For FITL, worst-case cost is O(N × D) ≈ 44 × 5 = 220 probe steps per call on affected templates, which fits inside the existing `maxDecisionProbeSteps=128` default after the Phase 1 head-level early-exit relaxation from 138ENUTIMTEM-002. Before declaring the fix complete, the spec requires a profiling gate: if the subset-extraction adds >25% wall-clock overhead to the 20-seed arvn sweep vs. pre-spec baseline, a memoization cache keyed by `(stateHash, actionId)` is added to `GameDefRuntime`; otherwise caching is deferred as YAGNI. A CI performance gate then enforces the <25% budget as a regression barrier.
...
- Baseline: checkout the commit prior to 138ENUTIMTEM-003's merge (use `git log` to identify), run `time node campaigns/fitl-arvn-agent-evolution/run-tournament.mjs --seeds 20 --players 4 --evolved-seat arvn --max-turns 200`, record wall-clock.
- Post-003: checkout HEAD (after 138ENUTIMTEM-004 merges), run the same command, record wall-clock.
...
- Add a `viableHeadSubsetCache?: LruMap<string, readonly MoveParamScalar[]>` slot to `GameDefRuntime`. Key format: `${stateHash}:${actionId}`.
...
- Wire into `prepare-playable-moves.ts`'s guided-chooser path: before calling `classifyDecisionSequenceSatisfiability`, check the cache; on miss, classify and populate the cache.
...
- Run the 20-seed arvn sweep twice within the test process: once with `disableGuidedChooser: true` (test-only flag from 003), once with guided chooser active.
...
- No changes to the guided chooser logic (owned by 138ENUTIMTEM-003).
...
5. 20-seed arvn sweep under guided chooser produces the same final stateHash as baseline for every seed where guided restriction did not occur (re-confirms 138ENUTIMTEM-003 T4 invariant post-caching).
