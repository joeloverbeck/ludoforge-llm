...
Per Spec 145 §I4 and the post-`145PREVCOMP-001` review split: prove engine-agnosticism (F#1) and decision-granularity uniformity (F#19) of the driver by running the same `pickInnerDecision` greedy policy against production action-selection microturns: FITL Govern, FITL March, FITL Train, FITL Sweep, FITL Assault, and Texas Hold'em raise. Assert each FITL action retires within `K_PREVIEW_DEPTH` under depthCap=8 and produces a non-pre-move-equal `selfMargin` (i.e., the driver actually moved state). Assert FITL March returns `depthCap` when the cap is lowered to 2, preserving the boundedness witness that `145PREVCOMP-001` originally named but only covered synthetically.
...
2. FITL Govern, March, Train, Sweep, Assault, and Texas Hold'em raise actions are kernel-published action-selection microturns in the corresponding `data/games/<game>/` GameSpecDocs. No new fixtures required — the test sets up minimal initial states for each game.
...
- Set up minimal FITL initial state (use `makeIsolatedInitialState` per existing convention).
- Identify action-selection microturns for ARVN `Govern`, `March`, `Train`, `Sweep`, and `Assault`.
...
- Assert for every action: `outcome.kind === 'ready'`; the post-drive `selfMargin` differs from the pre-move `selfMargin`; `outcome.depth <= K_PREVIEW_DEPTH`.
...
- Assert: `result1.state.stateHash === result2.state.stateHash`; `result1.depth === result2.depth`.
...
2. `pnpm -F @ludoforge/engine test:integration` green overall.
...
3. All cases assert `outcome.depth <= K_PREVIEW_DEPTH` (F#10).
