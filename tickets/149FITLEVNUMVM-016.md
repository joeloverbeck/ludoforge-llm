...
Ticket `150FITLWASM-004` delivered supported scalar candidate score rows but left full-profile library candidate-feature, aggregate, preview-backed, and dynamic rows fail-closed. Ticket `150FITLWASM-005` landed non-preview state-feature, candidate-feature, and aggregate score rows; ticket `150FITLWASM-006` landed preview-backed score-row parity but its same-seam preflight remained red at `6539.22 ms` per card. Ticket `150FITLWASM-007` landed production WASM score-row routing, but the gate remained red at `7131.37 ms` per card with `wasmScoreRowRouteCount=65` and `wasmScoreRowUnsupportedCount=0`. Ticket `150FITLWASM-008` is now the active WASM successor gate before this F14 cut may execute.
