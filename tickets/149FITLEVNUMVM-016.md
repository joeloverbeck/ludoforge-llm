...
**Status**: BLOCKED by Phase 5/WASM successor gate `tickets/150FITLWASM-029.md` — final F14 default-flip/deletion owner
**Priority**: HIGH
**Effort**: Large
...
**Deps**: `archive/tickets/149FITLEVNUMVM-014.md`, `archive/tickets/149FITLEVNUMVM-015.md`, `archive/tickets/149FITLEVNUMVM-018.md`, `tickets/149FITLEVNUMVM-022.md`, `archive/tickets/150FITLWASM-013.md`, `archive/tickets/150FITLWASM-014.md`, `archive/tickets/150FITLWASM-010.md`, `tickets/150FITLWASM-029.md`

## Problem

Phase 4's terminal deliverable owns the F14 cut: default the policy evaluation path to bytecode VM and delete the closure-tree runtime. Ticket 018 ruled out the engine-test workflow lanes as the remaining runtime blocker after stale golden fallout was repaired, but later profiling proved the remaining one-card wall time is dominated by generic preview-drive runtime work outside the current policy bytecode VM. Ticket 022 then proved Phase 4B still misses the original budget after its runtime-closure slices; Spec 150 now owns the Phase 5/WASM successor path required before this ticket can execute. Ticket `150FITLWASM-001` landed the skeleton, ticket `150FITLWASM-002` landed supported policy-bytecode value parity, ticket `150FITLWASM-003` landed the supported encoded-state/action batch bridge, ticket `150FITLWASM-004` landed supported scalar candidate score rows, ticket `150FITLWASM-005` landed non-preview score rows, ticket `150FITLWASM-006` landed preview-backed score-row parity, ticket `150FITLWASM-007` landed active production WASM score-row routing, ticket `150FITLWASM-008` landed score-row setup caching, ticket `150FITLWASM-009` landed preview-state/surface row materialization, ticket `150FITLWASM-012` landed FITL-current encoded preview-drive inventory support, archived ticket `150FITLWASM-013` landed encoded preview-state slot inventory support, ticket `150FITLWASM-014` landed the generic production preview-drive substrate, ticket `150FITLWASM-010` landed preview-drive production routing and fail-closed-clean route activation, ticket `150FITLWASM-015` landed route-local literal overhead cleanup, ticket `150FITLWASM-016` landed a generic hash-cache slice, ticket `150FITLWASM-017` landed active-route query-materialization runtime reuse, ticket `150FITLWASM-018` landed active-route token-index/digest cleanup, ticket `150FITLWASM-019` landed exact shared FNV hashing, ticket `150FITLWASM-020` landed generic token-hash and encoded-input reductions while leaving the gate red around `2.5 s`, ticket `150FITLWASM-021` landed a setup-hash root-counter reduction while leaving the gate red around `2.5 s`, ticket `150FITLWASM-022` landed bounded dynamic Zobrist feature-key memoization while leaving the gate red at `2539.8 ms`, ticket `150FITLWASM-023` landed apply-move token-placement hash deferral while leaving the gate red at `2557.17 ms`, ticket `150FITLWASM-024` landed initial full-hash runtime-table cache reuse while leaving the gate red at `2467.29 ms`, ticket `150FITLWASM-025` landed generic FNV prefix-state reuse while leaving the gate red at `2375.99 ms`, ticket `150FITLWASM-026` landed a run-local pending-request fingerprint cache while leaving the gate red at `2408.84 ms`, ticket `150FITLWASM-027` landed a generic namespace-prefix stable-fingerprint hasher while leaving the gate red at `2477.81 ms`, ticket `150FITLWASM-028` landed generic query/spatial allocation reductions and cached WASM layout encoding while leaving the gate red in the low `~2.1 s` range, and ticket `150FITLWASM-029` now owns remaining allocation, encoding, query/eval, token-index, decision-stack digest, and hash/canonicalization closure.
...
Ticket `150FITLWASM-004` delivered supported scalar candidate score rows but left full-profile library candidate-feature, aggregate, preview-backed, and dynamic rows fail-closed. Ticket `150FITLWASM-005` landed non-preview state-feature, candidate-feature, and aggregate score rows; ticket `150FITLWASM-006` landed preview-backed score-row parity but its same-seam preflight remained red at `6539.22 ms` per card. Ticket `150FITLWASM-007` landed production WASM score-row routing, but the gate remained red at `7131.37 ms` per card with `wasmScoreRowRouteCount=65` and `wasmScoreRowUnsupportedCount=0`. Ticket `150FITLWASM-008` landed score-row setup caching, but the gate remained red at `6593.68 ms` per card with `wasmScoreRowRouteCount=65`, `wasmScoreRowUnsupportedCount=0`, and `wasmScoreRowBytecodeCompileCount=42`. Ticket `150FITLWASM-009` landed preview-state/surface row materialization, but the gate remained red at `6632.26 ms` per card with `wasmPreviewCandidateFeatureRowRouteCount=77` and `wasmPreviewCandidateFeatureRowUnsupportedCount=0`. Archived ticket `150FITLWASM-013` completed encoded preview-state slot inventory support, and ticket `150FITLWASM-014` completed the generic production preview-drive substrate. Ticket `150FITLWASM-010` completed production preview-drive routing and fail-closed-clean route activation, but the gate remained red at `4124.29 ms`; ticket `150FITLWASM-015` landed route-local literal cleanup but left the gate red at `3958.91 ms`; ticket `150FITLWASM-016` landed a generic hash-cache slice but left the gate red at `4018.94 ms`; ticket `150FITLWASM-017` landed active-route query-materialization runtime reuse but left the gate red at `2898.06 ms`; ticket `150FITLWASM-018` landed active-route token-index/digest cleanup but left the gate red at `2761.91 ms`; ticket `150FITLWASM-019` landed exact shared FNV hashing but left the gate red at `2460.65 ms`; ticket `150FITLWASM-020` landed generic token-hash and encoded-input reductions but left the gate red around `2.5 s`; ticket `150FITLWASM-021` landed a setup-hash root-counter reduction but left the gate red around `2.5 s`; ticket `150FITLWASM-022` landed bounded dynamic Zobrist feature-key memoization but left the gate red at `2539.8 ms`; ticket `150FITLWASM-023` landed apply-move token-placement hash deferral while leaving the gate red at `2557.17 ms`; ticket `150FITLWASM-024` landed initial full-hash runtime-table cache reuse while leaving the gate red at `2467.29 ms`; ticket `150FITLWASM-025` landed generic FNV prefix-state reuse while leaving the gate red at `2375.99 ms`; ticket `150FITLWASM-026` landed a run-local pending-request fingerprint cache but left the gate red at `2408.84 ms`; ticket `150FITLWASM-027` landed a generic namespace-prefix stable-fingerprint hasher but left the gate red at `2477.81 ms`; ticket `150FITLWASM-028` landed generic query/spatial allocation reductions and cached WASM layout encoding but left the gate red in the low `~2.1 s` range; ticket `150FITLWASM-029` now owns the final active-route perf gate for this F14 cut.
