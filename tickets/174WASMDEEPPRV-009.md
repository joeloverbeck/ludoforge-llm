...
2. Confirmed activation counters (`getProductionPolicyWasmPreviewDriveRouteCount`, `getProductionPolicyWasmPreviewDriveUnsupportedCount`) are exported from `policy-wasm-runtime.ts` after ticket 001 lands; broad counter activation is handled by ticket 008, deep state-patch ABI substrate is prerequisite-owned by ticket 012, and full deep activation is owned by ticket 011.
