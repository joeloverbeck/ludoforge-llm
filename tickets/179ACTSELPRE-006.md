...
This ticket is **optional** per spec §10 — descope path documented in Out of Scope below. The default close condition is "documented fallback (path b)" if witness data from ticket 005 shows the WASM path is rarely active on opt-in profiles.
...
3. Ticket 005's Phase 2 witness uses the TS path by default (per Spec 178 reports indicating WASM falls back to TS for most decisions). The question of how often WASM is the active route on opt-in profiles is empirical.

**Gate condition**: Close this ticket via path (b) — documented TS fallback — if `tickets/179ACTSELPRE-005.md`'s Phase 2 witness data shows the WASM path was active on <10% of opt-in-profile decisions. The cost of path (a) — actively mirroring the opt-in in WASM — exceeds the value when WASM is rarely the runtime.
...
3. Update `reports/179-phase-2-post-opt-in-witness.md` (ticket 005) with WASM-active-decision percentages.
...
3. Update `docs/agent-dsl-cookbook.md`'s opt-in documentation (added by ticket 005) with: "Note: profiles opting into `outcomeGrantContinuation` always use the TS preview-drive route. WASM acceleration is disabled for opt-in decisions."

### Default selection rule

Choose path (b) unless ticket 005's witness data shows WASM was active on ≥ 10% of `arvn-evolved` decisions. If WASM is rarely active anyway, path (b) is correct (and cheaper). If WASM is materially active, path (a) prevents perf regression for opt-in profiles.
...
- **Descope path**: If ticket 005's witness shows WASM was active on <10% of opt-in decisions AND no operator preference for path (a) is expressed, close this ticket with "Declined — TS fallback documented in ticket 005's cookbook addendum; explicit WASM early-return deferred until WASM-route activity exceeds 10% on opt-in profiles." Outcome: no code change; cookbook addendum from ticket 005 carries the operator-facing contract.
...
2. Ticket 005's cookbook addendum carries the WASM-route documentation.
