...
5. `maxCandidatesPerRole` is the existing Foundation #10 cap that bounds `rejectedByConstraint` length per the spec. Verify the exact symbol name in `types-core.ts` or the role-binding subsystem during implementation.
...
2. **Foundation #10 (Bounded Computation)**: `rejectedByConstraint` inherits the existing `maxCandidatesPerRole` cap. The trace records `…+N more` when truncated. No new unbounded enumeration surface.
...
In `packages/engine/src/agents/plan-proposal.ts`, during the candidate-iteration loop, accumulate rejection records and attach them to the alternative being emitted. Bound the per-alternative list by `maxCandidatesPerRole`; when truncated, append a synthetic `…+N more` entry OR record the truncation count as part of the alternative's trace (TBD during implementation — prefer the truncation count as a separate field for cleaner semantics).
...
Asserts: across the FITL conformance fixture set, `rejectedByConstraint.length ≤ maxCandidatesPerRole` for every alternative; truncation (when triggered) is recorded. Mark with `// @test-class: architectural-invariant`.
...
2. `plan-trace-rejected-by-constraint-bounded.test.ts` (new) — bounded by `maxCandidatesPerRole`; truncation recorded.
...
3. `rejectedByConstraint.length ≤ maxCandidatesPerRole` for every alternative.
