...
Refactor the token rendering call path so the renderer can resolve zone token layouts using actual zone metadata, not just `zoneID`. That likely requires changes in:
...
If the cleanest approach is to pass rendered zones into `TokenRenderer.update()`, do that directly rather than inventing a second bespoke layout map.
...
5. If the token renderer contract changes, `packages/runner/test/canvas/GameCanvas.test.ts` verifies `GameCanvas` passes the new zone/token-layout inputs correctly.
...
2. `packages/runner/test/canvas/GameCanvas.test.ts` — only if needed for changed renderer input plumbing
