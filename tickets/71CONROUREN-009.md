...
1. The recently completed FITL migration (`71CONROUREN-006`) established that explicit route topology belongs in visual config, not in zone-id parsing heuristics. Confirmed.
2. The remaining limitation is not a GameSpecDoc correctness bug by itself. FITL route names such as `Hue-Khe Sanh`, `Saigon-Da Lat`, or `Can Tho-Chau Doc` can be historically meaningful labels even when the named town is not modeled as a standalone board zone in the current `GameDef`.
3. The current runner contract still forces endpoint pairs to be existing neighboring zone IDs. That is an architectural constraint of the renderer pipeline, not a proof that the authored FITL data is wrong.
4. The mismatch is therefore visual-topology expressiveness, not kernel semantics. Fixing it belongs in runner visual config / presentation architecture, not in the engine or in ad hoc FITL aliases.
...
The exact schema may differ, but the contract must distinguish zone-backed anchors from pure visual anchors.
...
- `tickets/71CONROUREN-009.md` (new)
...
4. Connection routes still fail closed when endpoint geometry is invalid or missing.
5. FITL production visual config no longer needs proxy board-zone endpoints for routes whose canonical labels refer to non-zone towns/junctions.
...
2. `packages/runner/test/config/validate-visual-config-refs.test.ts` — proves anchor and endpoint references are validated strictly.
3. `packages/runner/test/presentation/connection-route-resolver.test.ts` — proves mixed zone/anchor endpoint resolution works and invalid anchor geometry fails closed.
4. `packages/runner/test/presentation/presentation-scene.test.ts` — proves the scene path consumes provider-owned anchor geometry correctly.
5. `packages/runner/test/config/visual-config-files.test.ts` — proves FITL production config uses the generic anchor contract as intended.

### Commands

1. `pnpm -F @ludoforge/runner exec vitest run test/config/visual-config-schema.test.ts`
2. `pnpm -F @ludoforge/runner exec vitest run test/config/validate-visual-config-refs.test.ts`
3. `pnpm -F @ludoforge/runner exec vitest run test/presentation/connection-route-resolver.test.ts`
4. `pnpm -F @ludoforge/runner exec vitest run test/presentation/presentation-scene.test.ts`
5. `pnpm -F @ludoforge/runner exec vitest run test/config/visual-config-files.test.ts`
