...
5. No other active Spec 95 ticket owns this cleanup better than ticket `010`. Tickets `003` and `006` are adjacent, but neither should absorb a broader cross-layer contract-centralization refactor. Confirmed.
...
3. New/updated tests prove validator and compiler consume the same canonical `profile.use` bucket mapping for library-id validation/lowering.
4. No duplicated local allowlists or ad hoc bucket-mapping tables remain for the targeted policy vocabularies in validator/compiler/schema code.
5. Existing suite: `pnpm -F @ludoforge/engine test` — all pass
6. Existing suite: `pnpm turbo typecheck` — all pass
7. Existing suite: `pnpm turbo lint` — all pass
