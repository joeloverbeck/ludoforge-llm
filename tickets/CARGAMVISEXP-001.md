# CARGAMVISEXP-001: Token type prefix matching (fix gray circles)
...
`getTokenTypeVisual()` in `visual-config-provider.ts` does exact key lookup on `config.tokenTypes[tokenTypeId]`. Texas Hold'em has 52 token types (`card-2S` through `card-AS`) but no individual `tokenTypes` entries, so the fallback is a gray circle. The fix is a `tokenTypeDefaults` array in the visual config schema that matches token IDs by prefix.
...
1. Prefix matching is a generic visual config feature — any game can use `tokenTypeDefaults` with any prefix pattern. No game-specific branching.
...
### 2. Add prefix fallback to three provider methods

In `visual-config-provider.ts`:
- Extract a private helper `findDefaultByPrefix(tokenTypeId: string)` that iterates `config.tokenTypeDefaults` and returns the first entry whose `match.idPrefixes` includes a prefix of `tokenTypeId`.
- In `getTokenTypeVisual()`: after exact-key miss, call `findDefaultByPrefix()` and return its `style` if found.
- In `resolveTokenSymbols()`: after exact-key miss, fall back to default's style symbols.
- In `getTokenTypeDisplayName()`: after exact-key miss, fall back to default's style `displayName` if present.
...
1. `visual-config-provider.test.ts` — new test: prefix match returns card style for `card-2S` when `tokenTypeDefaults` has `idPrefixes: ['card-']`
2. `visual-config-provider.test.ts` — new test: exact match takes priority over prefix default (explicit `card-2S` entry overrides the prefix-matched default)
3. `visual-config-provider.test.ts` — new test: first matching prefix wins when multiple defaults match
4. `visual-config-provider.test.ts` — new test: returns default (circle) when no exact match AND no prefix match
5. `visual-config-provider.test.ts` — new test: `resolveTokenSymbols()` falls back to prefix default symbols
6. `visual-config-provider.test.ts` — new test: `getTokenTypeDisplayName()` falls back to prefix default displayName
7. Existing suite: `pnpm -F @ludoforge/runner test`

### Invariants

1. Exact-match `tokenTypes` entries always take priority over `tokenTypeDefaults` prefix matches.
...
1. `packages/runner/test/config/visual-config-provider.test.ts` — add describe block for prefix matching: exact priority, first-prefix-wins, no-match fallback, resolveTokenSymbols fallback, getTokenTypeDisplayName fallback
