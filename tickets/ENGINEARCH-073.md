...
2. Export-surface analysis helper logic is shared and now a foundational dependency for API boundary guard tests.
3. There is currently no dedicated helper test file validating export-shape edge cases (named re-exports, aliasing, wildcard re-exports, default export forms).
4. **Mismatch + correction**: shared guard helpers require direct tests to preserve guard correctness as helper logic evolves.
...
Create a dedicated test file for `kernel-source-ast-guard.ts` that parses synthetic TS snippets and verifies expected export metadata behavior.
...
- `packages/engine/test/unit/helpers/kernel-source-ast-guard.test.ts` (new)
...
1. `packages/engine/test/unit/helpers/kernel-source-ast-guard.test.ts` — direct helper contract tests across export-shape edge cases.

### Commands

1. `pnpm -F @ludoforge/engine build`
2. `node --test packages/engine/dist/test/unit/helpers/kernel-source-ast-guard.test.js`
