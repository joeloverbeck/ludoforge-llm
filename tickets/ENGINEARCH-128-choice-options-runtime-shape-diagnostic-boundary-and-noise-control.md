...
3. Existing active tickets do not explicitly cover this boundary/noise issue:
   - `ENGINEARCH-111` targets inferencer surface unification.
   - `ENGINEARCH-127` targets query type ownership boundaries.
   Corrected scope: keep shared shape-invariant logic while restoring strict layer-owned diagnostics and deterministic validator diagnostic policy.
...
1. `packages/engine/test/unit/kernel/choice-options-runtime-shape-contract.test.ts` — assert semantic violation output contract without layer-specific diagnostic-code coupling.
