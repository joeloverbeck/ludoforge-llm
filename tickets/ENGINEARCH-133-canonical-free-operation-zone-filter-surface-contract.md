...
3. Mismatch: duplicated union ownership is not a clean architecture contract; corrected scope is to define one canonical shared type and import it from all consumers.
...
Define one canonical exported `FreeOperationZoneFilterSurface` contract in a shared kernel type owner and remove duplicate local unions.
...
Add/extend tests to guard that legal/probe and strict/apply surfaces still map to expected behavior and diagnostics under the canonical contract.

## Files to Touch

- `packages/engine/src/kernel/missing-binding-policy.ts` (modify)
- `packages/engine/src/kernel/turn-flow-error.ts` (modify)
- `packages/engine/src/kernel/turn-flow-eligibility.ts` (modify, if type import changes)
- `packages/engine/src/kernel/eval-query.ts` (modify, if type import changes)
- `packages/engine/test/unit/kernel/missing-binding-policy.test.ts` (modify)
- `packages/engine/test/unit/kernel/legal-moves.test.ts` (modify/add if diagnostics coverage needs updates)
...
3. `packages/engine/test/unit/kernel/legal-moves.test.ts` — lock probe-surface deferral behavior.
