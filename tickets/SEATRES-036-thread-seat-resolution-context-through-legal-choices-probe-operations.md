...
2. Thread the context through helper chains that call free-operation discovery/validation.
...
1. `packages/engine/test/unit/legal-choices.test.ts` — add lifecycle ownership regression coverage for free-operation probing paths; ensure behavior parity.
2. `packages/engine/test/unit/legal-choices.test.ts` — add guard that probe/evaluation flow does not require nested context reconstruction patterns.
